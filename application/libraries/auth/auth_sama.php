...
/////////////////////////////khusus group_id = 6//////////////////////>>
    function get_user_unor($user_id = false)
    {
		...
		$this->CI->db->from('m_unor');
		$this->CI->db->where_in('id_unor',$b_unor);
		$this->CI->db->order_by('kode_unor','asc');
      	$qry = $this->CI->db->get()->row();
		$id_unor = $qry->id_unor;
		$kode_unor = $qry->kode_unor;
		$nama_unor = $qry->nama_unor;

		$b_unor = explode(",",$b_unor);
		$acl = json_encode($b_unor);

      return array('nama_unor'=>$nama_unor,'kode_unor'=>$kode_unor,'id_unor'=>$id_unor,'acl'=>$acl);
    }
...
//			$sqlstr="SELECT a.*, b.* FROM users a LEFT JOIN (cmf_setting b) ON (a.group_id=b.id_item) WHERE a.username = '$username' AND a.status='on'";
//			$query=$this->CI->db->query($sqlstr);
			$this->CI->db->from('users a');
			$this->CI->db->join('cmf_setting b','a.group_id=b.id_item','left');
			$this->CI->db->where('a.username',$username);
//			$this->CI->db->where('a.passwd',$password);
			...
			switch ($group_name):
				case 'pengelola':
					...
					$this->CI->session->set_userdata('kode_unor', $unor['kode_unor']);
					$this->CI->session->set_userdata('id_unor', $unor['id_unor']);
					break;
				case 'absensor_unit':
	                $unor = $this->get_user_unor($id_user);
					$this->CI->session->set_userdata('user_id', $id_user);
