			...
			<table class="table table-striped table-bordered table-hover">
				<thead id=gridhead>
					<tr height=20>
						...
						<?php if($ibel->status!="aju" && $ibel->status!="koreksi" && $ibel->status!="acc"){ ?>
...
function save_jawaban(){
		...
		var jawaban = $("input[name='jawaban']").val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/karis_karsu_proses/save_jawaban",
...
function edit_jawaban(idd){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/karis_karsu_proses/ini_catatan",
				data:{"id_catatan":idd},
				beforeSend:function(){	
