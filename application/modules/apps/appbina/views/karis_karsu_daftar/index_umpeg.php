...
function setForm(tujuan,idd,no){
	var kop = []; 
	kop['form_tambah'] = "FORM TAMBAH PEGAWAI PEMOHON KARIS/KARSU"; 
	kop['form_suris'] = "FORM KARIS/KARSU"; 
	kop['form_hapus'] = "FORM HAPUS PEGAWAI PEMOHON"; 
	var act = []; 
	act['form_tambah'] = "<?=site_url();?>appbina/karis_karsu_daftar/tambah_aksi"; 
	act['form_suris'] = "<?=site_url();?>appbina/karis_karsu_daftar/suris_aksi"; 
	act['form_hapus'] = "<?=site_url();?>appbina/karis_karsu_daftar/hapus_aksi"; 
	var btt = [];
	...
	btt['form_suris'] = "<button id='btAct' type=sumbit class='btn btn-primary' onclick='ajukan(); return false;'><i class='fa fa-save fa-fw'></i> Simpan</button>"; 
	btt['form_hapus'] = "<button id='btAct' type=sumbit class='btn btn-danger' onclick='ajukan(); return false;'><i class='fa fa-trash fa-fw'></i> Hapus</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appbina/karis_karsu_daftar/"+tujuan,
			data:{"idd": idd,"no": no },
			beforeSend:function(){	
				$("#pageKonten").hide();
...
function gridpagingA(hal){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbina/karis_karsu_daftar/getdata",
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_suris\','+item.id_karis_karsu+','+no+');return false;"><i class="fa fa-pencil fa-fw"></i> Edit data</a></li>';
						}
						if(item.hapus=="ya"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="setForm(\'form_hapus\','+item.id_karis_karsu+','+no+');return false;"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						}
						if(item.hapus=="ya" || item.status=="draft"){
						table = table+ '<li role="presentation" class="divider">';
						}
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_karis_karsu+');return false;"><i class="fa fa-search fa-fw"></i> Lihat detail</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
//					table = table+ "<td style='padding:3px;'><b>"+item.nama_pegawai+"</b> ("+item.gender+")<br/>"+item.tempat_lahir+", "+item.tanggal_lahir+"<br/>"+item.nip_baru+" / "+item.tmt_pns+"</td>";
					...
					table = table+ "</div></td>";
					table = table+ "<td><div>";
					table = table+ "<div style='float:left; width:65px;'>Tg. nikah</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'><b>"+item.tg_menikah+"</b></div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Nama</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'>"+item.nama_suris+"</div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>TT. lahir</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'>"+item.tempat_lahir_suris+", "+item.tg_lahir_suris+"</div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Pendidikan</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<span><div style='display:table;'>"+item.pendidikan_suris+"</div></span>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Pekerjaan</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<span><div style='display:table;'>"+item.pekerjaan_suris+"</div></span>";
					table = table+ "</div></td>";
					table = table+ "<td>";
					...
					table = table+ '<a href="#" class="label label-default" onclick="zoom_dok(\'karis_karsu\','+item.id_peg_perkawinan+',\''+item.nip_baru+'\');return false;"><i class="fa fa-search fa-fw"></i> Zoom</a>';
										}
					table = table+ "</p>";
					table = table+ "</div>";
					...
					table = table+ "</div>";
									table = table+ "<div>";
									table = table+ "<div style='float:left; width:135px;'>Nomor</div>";
									table = table+ "<div style='float:left; width:10px;'>:</div>";
									table = table+ "<div style='float:left;'>"+accIb.nomor_surat+"</div>";
									table = table+ "</div>";
									table = table+ "<div style='clear:both'>";
									table = table+ "<div style='float:left; width:135px;'>Tanggal</div>";
									table = table+ "<div style='float:left; width:10px;'>:</div>";
...
function ppost(idd,act){
	...
	$('#sb_act').attr('action','<?=site_url();?>appbina/karis_karsu_edok/alih');
	...
	var tab = tab + '<input type="hidden" name="asal" value="appbina/karis_karsu_daftar">';
