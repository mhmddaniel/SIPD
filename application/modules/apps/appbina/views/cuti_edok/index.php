...
<div class="row">
	...
	<div class="col-lg-6">
		<div class="panel panel-default">
			<div class="panel-heading"><i class="fa fa-upload fa-fw"></i> <b>Pengajuan</b></div>
			<div class="panel-body">
								<div style="clear:both">
										<div style="float:left; width:90px;">Jenis Cuti</div>
										<div style="float:left; width:5px;">:</div>
										<span><div style="display:table;"><?=$jenis[$val->kode_jenis_cuti];?></div></span>
								</div>
								<?php 
									if($val->kode_tujuan!=""){
								?>
								<div style="clear:both">
										<div style="float:left; width:90px;">Tujuan</div>
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$val->kode_tujuan;?></div>
								</div>
								<?php 
									}
								?>
								<div style="clear:both">
										<div style="float:left; width:90px;">Mulai Cuti</div>
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$val->tanggal_mulai_cuti;?></div>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Selesai Cuti</div>
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$val->tanggal_sampai_cuti;?></div>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Untuk</div>
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$val->alasan_cuti;?></div>
								</div>
			</div>
		</div><!-- /.panel -->
	</div><!-- /.col-lg-6 -->
...
<div id="accordion" class="panel-group" style="padding-bottom:30px;">
<?php
	$no=0;
	foreach($kode_dokumen_cuti AS $key=>$vl){
		if($no>0){
			if($no==1){
				$clss="in"; 
				$sty="";
...
?>
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4 class="panel-title"><a class="collapsed" href="#collapseOne<?=$key;?>" data-parent="#accordion" data-toggle="collapse" onclick="isi('<?=$key;?>','_edok');" id="tbtg_<?=$key;?>"><?=$no;?>. <?=$vl;?></a></h4>
		</div>
...
?>
	<div class="panel panel-danger">
		<div class="panel-heading">
			<h4 class="panel-title"><a class="collapsed" href="#collapseOnejawaban" data-parent="#accordion" data-toggle="collapse" onclick="isi('jawaban','_proses');" id="tbtg_catatan"><?=$no;?>. Catatan Pemroses</a></h4>
		</div>
		<div id="collapseOnejawaban" class="panel-collapse collapse <?=$clss;?>" style="<?=$sty;?>">
			<div class="panel-body"><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></div>
		</div>
	</div>
	
	<?php
	if($val->status=="draft" || $val->status=="revisi"){
	?>
	<div class="panel panel-warning">
		<div class="panel-heading">
			<h4 class="panel-title"><a class="collapsed" href="#collapseOneajukan" data-parent="#accordion" data-toggle="collapse" onclick="isi('ajukan','');" id="tbtg_catatan"><?=$no+1;?>. Ajukan Permohonan</a></h4>
		</div>
...
function uppl(bttn,stts,dokumen){	
		...
		new AjaxUpload(btnUpload, {
			action: '<?=site_url();?>appbina/cuti_edok/saveupload',
			name: 'artikel_file',
...
function ok_hapus(tipe,idd){
	...
	var id_dokumen = $('#head_no_'+tipe).attr("data-id_dokumen");
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti_edok/hapus_dok",
...
function isi(dok,prs){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti"+prs+"/"+dok,
...
function viewForm(komponen){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti_edok/"+komponen+"_formedit",
				data:{"id_pegawai":"<?=$id_pegawai;?>"},
				beforeSend:function(){	
					$('#accordion').hide();
					$('#formedok').show();
...
function viewUppl(komponen,idd){
	...
	var nip_baru = $('#nip_baru').html();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti_edok/"+komponen+"_upload",
...
function satuket(idd){
	...
	var sub_keterangan = $('#sub_ket_dok_'+idd).val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti_edok/edit_keterangan_aksi",
...
function zoom_dox(komponen,idd){
	var nip_baru = $('#nip_baru').html();
	$('#sb_act').attr('action','<?=site_url();?>appbina/cuti_edok/zoom').attr('target','_blank');
	var tab = '<input type="hidden" name="komponen" value="'+komponen+'">';
	var tab = tab + '<input type="hidden" name="nip_baru" value="'+nip_baru+'">';	
	$('#sb_act').html(tab).submit();
}
