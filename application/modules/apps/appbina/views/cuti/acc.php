								...
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span id="notif"  ></span> <span class="fa fa-tasks fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											...
											<li role="presentation" onClick="setForm('form_tambah','xx','1');return false;"><a href="#" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-sort-amount-desc fa-fw"></i> Tambah Pegawai Pemohon Cuti</a></li>
											<li role="presentation"><a href="<?=site_url('module/appbina/cuti');?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-sort-amount-desc fa-fw"></i> Daftar Pengajuan Cuti</a></li>
											
											<li role="presentation"><a href="<?=site_url('module/appbina/koreksi');?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-bell fa-fw"></i> Pengajuan yang Dikembalikan <span id="notif1" class="text-danger"></span></a></li>
											
											<li role="presentation"><a href="<?=site_url('module/appbina/acc');?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-bell fa-fw"></i> Pengajuan yang Diterima <span id="notif2" class="text-danger"></span></a></li>
...
function setForm(tujuan,idd,no){
	...
	btt['form_terima'] = "<button id='btAct' type=sumbit class='btn btn-primary' onclick='ajukan(); return false;'><i class='fa fa-save fa-fw'></i> Terima</button>";	
...
function gridpagingA(hal){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbina/cuti/getdata2",
		...
		success:function(data){
			if((data.hslquery.length)>0){
				...
				document.getElementById("notif").innerHTML = "+ " +(data.hslquery.length + data.hslquery3.length);
				
				if((data.hslquery.length || data.hslquery3.length)>0){
					jQuery('#ddMenuT').removeClass("btn btn-primary dropdown-toggle btn-xs").addClass("btn btn-danger dropdown-toggle btn-xs"); 
				}
				
				if((data.hslquery.length)!=null){
					if((data.hslquery.length)>0){
						document.getElementById("notif2").innerHTML = "+ " +(data.hslquery.length);
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_edit\','+item.id_cuti+','+no+');return false;"><i class="fa fa-pencil fa-fw"></i> Edit data</a></li>';
						}
						if(item.hapus=="ya"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="setForm(\'form_hapus\','+item.id_cuti+','+no+');return false;"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						}
						if(item.hapus=="ya" || item.status=="draft"){
						table = table+ '<li role="presentation" class="divider">';
						}
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_cuti+');return false;"><i class="fa fa-search fa-fw"></i> Lihat detail</a></li>';
						if(item.status=="acc"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_terima\','+item.id_cuti+','+no+');return false;"><i class="fa fa-check fa-fw"></i> Terima</a></li>';
						}
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
...
//					table = table+ "<td style='padding:3px;'><b>"+item.nama_pegawai+"</b> ("+item.gender+")<br/>"+item.tempat_lahir+", "+item.tanggal_lahir+"<br/>"+item.nip_baru+" / "+item.tmt_pns+"</td>";
			...
			} else if((data.hslquery3.length)>0) {
				...
				document.getElementById("notif2").innerHTML = "";
				jQuery('#ddMenuT').removeClass("btn btn-danger dropdown-toggle btn-xs").addClass("btn btn-primary dropdown-toggle btn-xs"); 
				$('#listA').html("<tr id=isi class=gridrow><td colspan=8 align=center><b>Tidak ada data</b></td></tr>");
				$('#pagingA').html("");
			} // end if
...
function ppost(idd,act){
	...
	$('#sb_act').attr('action','<?=site_url();?>appbina/cuti_edok/alih');
	...
	var tab = tab + '<input type="hidden" name="asal" value="appbina/cuti">';
