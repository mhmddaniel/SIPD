								...
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span id="notif"></span><span class="fa fa-tasks fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											...
											<li role="presentation"><a href="<?=site_url('module/appbina/aju');?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-bell fa-fw"></i> Pengajuan Cuti <span id="notif1" class="text-danger"></span></a></li>
...
function gridpagingA(hal){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbina/cuti/getdata3",
		...
		success:function(data){
			if((data.hslquery.length)>0){
				...
				hasil=data.hslquery.length;
				
				if(hasil!=0){
					document.getElementById("notif").innerHTML = "+ " +(data.hslquery.length);
				}else{
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="ppost('+item.id_cuti+',\'cuti_injek/edit\');"><i class="fa fa-pencil fa-fw"></i> Edit Data</a></li>';
						if(accIb.thumb=="assets/file/foto/photo.jpg"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_cuti+',\'cuti_injek/hapus\');"><i class="fa fa-trash fa-fw"></i> Hapus Data</a></li>';
						}
						table = table+ "</ul>";
						table = table+ "</div>";
						} else {
						table = table+ '<div class="btn btn-default btn-xs" onClick="ppost('+item.id_cuti+',\'cuti_proses/alih\');"><i class="fa fa-binoculars fa-fw"></i></div>';
						}
					table = table+ "</td>";
	//tombol aksi<--
					table = table+ "<td><div>";
					...
					table = table+ '<a href="#" class="label label-info" onclick="ppost('+item.id_cuti+',\'cuti_proses/upl\'); return false;"><i class="fa fa-upload fa-fw"></i> Atur Dokumen</a><br/>';
										}
										if(accIb.thumb!="assets/file/foto/photo.jpg"){
