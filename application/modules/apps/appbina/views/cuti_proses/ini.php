		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
								<div>
										...
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$jenis[$val->kode_jenis_cuti];?></div>
								</div>
								<?php 
									if($val->kode_tujuan!=""){
								?>
...
?>
	...
	<div class="panel panel-warning">
		<div class="panel-heading">
			<h4 class="panel-title"><?=$no+1;?>. <a class="collapsed" href="#collapseOneedit_sk" data-parent="#accordion" data-toggle="collapse" onclick="isi('edit_sk','proses');" id="tbtg_catatan">Persetujuan Permohonan</a></h4>
		</div>
		<div id="collapseOneedit_sk" class="panel-collapse collapse <?=$clss;?>" style="<?=$sty;?>">
			<div class="panel-body"><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></div>
		</div>
	</div>
	<?php
...
function isi(dok,prs){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti_"+prs+"/"+dok,
...
function isib(dok,prs){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/cuti_"+prs+"/not_"+dok,
				data:{"id_pegawai":"<?=$id_pegawai;?>"},
				beforeSend:function(){	
					$('#collapseOne'+dok).html('<p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p>');
				},
...
function viewForm(komponen){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/karis_karsu_edok/"+komponen+"_formedit",
				data:{"id_pegawai":"<?=$id_pegawai;?>"},
				beforeSend:function(){	
					$('#accordion').hide();
					$('#formedok').show();
...
function viewUppl(komponen,idd){
	...
	var nip_baru = $('#nip_baru').html();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/karis_karsu_edok/"+komponen+"_upload",
...
function satuket(idd){
	...
	var sub_keterangan = $('#sub_ket_dok_'+idd).val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbina/karis_karsu_edok/edit_keterangan_aksi",
