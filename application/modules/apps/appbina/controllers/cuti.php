...
///////////////////////////////////////////////////////////////////////////////////
  ...
  function kode_dokumen_cuti_diluar_tanggungan_negara($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['ijin'] = 'IJIN/PENGANTAR PIMPINAN';
    $select ['kartu_keluarga'] = 'KARTU KELUARGA';
    $select ['ktp_suami'] = 'KTP SUAMI';
    $select ['ktp_istri'] = 'KTP ISTRI';
    $select ['surat_nikah'] = 'SURAT NIKAH';
    $select ['skp'] = 'SKP 2 TAHUN TERAKHIR';
  ...
  function kode_dokumen_cuti_sakit($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['ijin'] = 'IJIN/PENGANTAR PIMPINAN';
    $select ['keterangan_dokter'] = 'SURAT KETERANGAN DOKTER';
    $select ['karpeg'] = 'KARTU PEGAWAI';
    $select ['konversi_nip'] = 'SK. KONVERSI NIP';
    $select ['sk_cpns'] = 'SK CPNS';
    $select ['sk_pns'] = 'SK PNS';
  ...
  function kode_dokumen_cuti_bersalin($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['ijin'] = 'IJIN/PENGANTAR PIMPINAN';
	$select ['keterangan_hamil'] = 'SURAT KETERANGAN HAMIL';
    $select ['kartu_keluarga'] = 'KARTU KELUARGA';
    $select ['buku_nikah_suami'] = 'BUKU NIKAH SUAMI';
    $select ['buku_nikah_istri'] = 'BUKU NIKAH ISTRI';
    $select ['karpeg'] = 'KARTU PEGAWAI';
    $select ['konversi_nip'] = 'SK. KONVERSI NIP';
    $select ['sk_cpns'] = 'SK CPNS';
    $select ['sk_pns'] = 'SK PNS';
  ...
  function kode_dokumen_cuti_tahunan($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['ijin'] = 'IJIN/PENGANTAR PIMPINAN';
    // $select ['pbb'] = 'PBB';
    // $select ['lunas_pbb'] = 'BUKTI LUNAS PBB';
  ...
  function kode_dokumen_cuti_alasan_penting($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['ijin'] = 'IJIN/PENGANTAR PIMPINAN';
	$select ['kartu_keluarga'] = 'KARTU KELUARGA';
	$select ['skp'] = 'SKP TERAKHIR';
  ...
  function kode_dokumen_cuti_besar($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['ijin'] = 'IJIN/PENGANTAR PIMPINAN';
    $select ['bpih'] = 'BUKTI SETORAN BPIH';
 //    $select ['pbb'] = 'LUNAS PBB';
	// $select ['lunas_pbb'] = 'BUKTI LUNAS PBB';
    $select ['karpeg'] = 'KARTU PEGAWAI';
    $select ['konversi_nip'] = 'SK. KONVERSI NIP';
    $select ['sk_cpns'] = 'SK CPNS';
...
///////////////////////////////////////////////////////////////////////////////////
	...
	function getdata(){
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_cuti->get_cuti($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			$data['hslquery2'] = $this->m_cuti->notif($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			$data['hslquery3'] = $this->m_cuti->notif2($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
	...
	function getdata2(){
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_cuti->notif2($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			$data['hslquery3'] = $this->m_cuti->notif($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
	...
	function getdata4(){
		...
		if($group_name=="pengelola"){
			...
			$bulan = (strlen($_POST['bulan'])==1)?"0".$_POST['bulan']:$_POST['bulan'];
			$tahun=$_POST['tahun'];
			$data['count'] = $this->m_cuti->hitung_cuti1($_POST['cari'],$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_cuti->get_cuti1($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			$data['hslquery3'] = $this->m_cuti->notif3($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			...
			foreach($data['hslquery'] AS $key=>$val){
				...
				$data['hslquery'][$key]->tg_aju = ($val->tg_aju==null)?"...":$val->tg_aju;
				$data['hslquery'][$key]->tg_koreksi = ($val->tg_koreksi==null)?"...":$val->tg_koreksi;
				$data['hslquery'][$key]->jenis_cuti = @$dCuti[$val->kode_jenis_cuti];
				$data['hslquery'][$key]->kode_jenis_tujuan = @$dTujuan[$val->kode_tujuan];
	...
	function getdataarsip(){
		...
		if($group_name=="pengelola"){
			...
			$bulan = (strlen($_POST['bulan'])==1)?"0".$_POST['bulan']:$_POST['bulan'];
			$tahun=$_POST['tahun'];
			$data['count'] = $this->m_cuti->hitung_cuti_acc($_POST['cari'],$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_cuti->get_cuti_acc($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			$data['hslquery3'] = $this->m_cuti->notif3($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
	...
	function getdata3(){
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_cuti->notif3($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
	...
	function ajukan(){
		...
		if($data['val']->kode_jenis_cuti==1){
			...
			$data['keterangan_dokter'] = $this->m_cuti->cek_dokumen($data['idd'],'keterangan_dokter');
		} else if($data['val']->kode_jenis_cuti==2 || $data['val']->kode_jenis_cuti==3 || $data['val']->kode_jenis_cuti==4) {	
			$data['ijin'] = $this->m_cuti->cek_dokumen($data['idd'],'ijin');
			$data['bpih'] = $this->m_cuti->cek_dokumen($data['idd'],'bpih');
			// $data['pbb'] = $this->m_cuti->cek_dokumen($data['idd'],'pbb');
			// $data['lunas_pbb'] = $this->m_cuti->cek_dokumen($data['idd'],'lunas_pbb');
		} else if($data['val']->kode_jenis_cuti==5) {	
			$data['ijin'] = $this->m_cuti->cek_dokumen($data['idd'],'ijin');
			$data['kartu_keluarga'] = $this->m_cuti->cek_dokumen($data['idd'],'kartu_keluarga');
		} else if($data['val']->kode_jenis_cuti==6) {	
			$data['ijin'] = $this->m_cuti->cek_dokumen($data['idd'],'ijin');
			$data['keterangan_hamil'] = $this->m_cuti->cek_dokumen($data['idd'],'keterangan_hamil');
			$data['buku_nikah_suami'] = $this->m_cuti->cek_dokumen($data['idd'],'buku_nikah_suami');
			$data['buku_nikah_istri'] = $this->m_cuti->cek_dokumen($data['idd'],'buku_nikah_istri');
			$data['kartu_keluarga'] = $this->m_cuti->cek_dokumen($data['idd'],'kartu_keluarga');
			...
			$data['ijin'] = $this->m_cuti->cek_dokumen($data['idd'],'ijin');
			$data['kartu_keluarga'] = $this->m_cuti->cek_dokumen($data['idd'],'kartu_keluarga');
			$data['ktp_suami'] = $this->m_cuti->cek_dokumen($data['idd'],'ktp_suami');
			$data['ktp_istri'] = $this->m_cuti->cek_dokumen($data['idd'],'ktp_istri');
			$data['surat_nikah'] = $this->m_cuti->cek_dokumen($data['idd'],'surat_nikah');
		}
	...
	function form_edit_admin(){
		...
		$data['val']->nama_pangkat = @$dWpangkat[$data['val']->kode_golongan];
		$data['val']->nama_golongan = @$dWgolongan[$data['val']->kode_golongan];
		$this->load->view('cuti/form_tambah_nip_admin',$data);
	}
	...
	function form_terima(){
		...
		$data['val']->nama_pangkat = @$dWpangkat[$data['val']->kode_golongan];
		$data['val']->nama_golongan = @$dWgolongan[$data['val']->kode_golongan];
		$this->load->view('cuti/form_terima',$data);
	}
	...
	function edit_aksi_admin(){
		$this->m_cuti->cuti_edit_admin($_POST);
		echo "sukses";
	}
	
	function terima_aksi(){
		$this->m_cuti->cuti_terima($_POST);
		echo "sukses";
	}
