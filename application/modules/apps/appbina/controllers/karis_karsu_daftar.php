...
class Karis_karsu_daftar extends MX_Controller {
...
///////////////////////////////////////////////////////////////////////////////////
  function tahapan_karis_karsu($asRef=false)  {
    $select ['buat'] = 'Pembuatan Pengajuan Karis/Karsu';
    $select ['draft'] = 'Pengisian Formulir Karis/Karsu';
    $select ['aju'] = 'Pengajuan Karis/Karsu';
    $select ['koreksi'] = 'Koreksi Pengajuan Karis/Karsu';
    $select ['revisi'] = 'Perbaikan Pengajuan Karis/Karsu';
    $select ['acc'] = 'Penerbitan Karis/Karsu';
    return $select;
  }
  function kode_dokumen_karis_karsu($asRef=false)  {
    if(!$asRef){	$select [''] = 'Pilih Jenis Dokumen';	}else{	$select [''] = '-';	}
    $select ['pasfoto'] = 'PASFOTO SUAMI/ISTRI';
...
///////////////////////////////////////////////////////////////////////////////////
	function index(){
		...
		$data['stib'] = Modules::run("appbangrir/karis_karsu_daftar/tahapan_karis_karsu");
		...
		$this->load->view('karis_karsu_daftar/'.$rd,$data);
	}
	...
	function getdata(){
		...
		if($group_name=="pengelola"){
			...
			$dWtahapan = $this->tahapan_karis_karsu();
			$bulan = (strlen($_POST['bulan'])==1)?"0".$_POST['bulan']:$_POST['bulan'];
			$tahun=$_POST['tahun'];
			$data['count'] = $this->m_karis_karsu->hitung_karis_karsu($_POST['cari'],$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_karis_karsu->get_karis_karsu($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stib);
			foreach($data['hslquery'] AS $key=>$val){
				...
				$data['hslquery'][$key]->hapus = $this->cek_dok($val->id_karis_karsu);
				$data['hslquery'][$key]->tg_aju = ($val->tg_aju==null)?"...":$val->tg_aju;
				$data['hslquery'][$key]->tg_koreksi = ($val->tg_koreksi==null)?"...":$val->tg_koreksi;

				$accIb = $this->m_karis_karsu->ini_karis_karsu($val->id_karis_karsu);
				if(empty($accIb)){
					@$acc['nomor_surat'] = "...";
					@$acc['tanggal_surat'] = "...";
					@$acc['editable'] = "yes";
					...
					@$acc['nomor_surat'] = $accIb->nomor;
					@$acc['tanggal_surat'] = date("d-m-Y", strtotime($accIb->tanggal));
					@$acc['editable'] = "yes";
					@$acc['thumb'] = $this->ini_thumb($val->nip_baru,$accIb->id_peg_perkawinan);
					@$acc['idd'] = $accIb->id_peg_karis_karsu;
				}
	...
	function ini_thumb($nip,$idd){
		$gbr = $this->m_edok->cek_dokumen($nip,"karis_karsu",$idd);
		$gambar = (empty($gbr))?"assets/file/foto/photo.jpg":"assets/media/file/".$nip."/karis_karsu/thumb_".$gbr[0]->file_dokumen;
		return $gambar;
	}

	function form_tambah(){
		$this->load->view('karis_karsu_daftar/form_tambah');
	}
	function cari_nip(){
			$this->load->model('appbkpp/m_umpeg');
			$user_id = $this->session->userdata('user_id');
		...
		$this->load->view('karis_karsu_daftar/form_tambah_nip',$data);
	}
	function tambah_aksi(){
		$this->db->from('r_pegawai_aktual');
		$this->db->where('id_pegawai',$_POST['id_pegawai']);
		$peg = $this->db->get()->row();
		$peg->id_peg_perkawinan = $this->m_karis_karsu->suris_tambah($_POST);
		$this->m_karis_karsu->tambah_pemohon($peg);
	...
	function form_hapus(){
			...
			$dWgolongan = $this->dropdowns->kode_golongan();
		$data['hapus'] = "ya";
		$data['val'] = $this->m_karis_karsu->ini_karis_karsu($_POST['idd']);
		$data['val']->nama_pangkat = @$dWpangkat[$data['val']->kode_golongan];
		$data['val']->nama_golongan = @$dWgolongan[$data['val']->kode_golongan];
		$this->load->view('karis_karsu_daftar/form_tambah_nip',$data);
	}
	function hapus_aksi(){
		$suris = $this->m_karis_karsu->ini_karis_karsu($_POST['id_karis_karsu']);
		$this->db->where('id_karis_karsu',$_POST['id_karis_karsu']);
		$this->db->delete('r_peg_karis_karsu_aju');
	...
	function ajukan(){
		...
		$data['pasfoto'] = $this->m_karis_karsu->cek_dokumen($data['idd'],'pasfoto');
		$data['buku_nikah_suami'] = $this->m_karis_karsu->cek_dokumen($data['idd'],'buku_nikah_suami');
		$data['buku_nikah_istri'] = $this->m_karis_karsu->cek_dokumen($data['idd'],'buku_nikah_istri');
		$data['catatan'] = $this->m_karis_karsu->get_catatan($data['idd'],'ditanya');

		$this->load->view('karis_karsu_daftar/form_ajukan',$data);
	}

	function ajukan_aksi(){
		$this->m_karis_karsu->ajukan_pemohon($_POST['id_karis_karsu']);
		echo "sukses";
	}
	...
	function suris_aksi(){
		$this->m_karis_karsu->suris_edit($_POST);
		echo "sukses";
	}
