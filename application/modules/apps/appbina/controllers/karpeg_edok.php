...
class Karpeg_edok extends MX_Controller {
...
///////////////////////////////////////////////////////////////////////////////////
	function alih(){
		...
		$karpeg = $this->m_karpeg->ini_karpeg($_POST['idd']);
		if($karpeg->status=="draft" && $karpeg->draft==null){	$this->m_karpeg->draft_pemohon($_POST['idd']);	}

		redirect("module/appbina/karpeg_edok");
	}
	function index(){
		...
		$this->load->view('karpeg_edok/index',$data);
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	function ijin(){
		...
		$data['komponen'] = "ijin";
		$data['karpeg'] = $this->m_karpeg->ini_karpeg($data['idd']);
		$data['surat'] = json_decode($data['karpeg']->ijin_pimpinan);

		$data['isi'] = $this->m_karpeg->cek_dokumen($data['idd'],$data['komponen']);
		if(!empty($data['isi'])){
			$ffl = "assets/media/karpeg/".$peg->nip_baru."/".$data['komponen']."/".$data['isi'][0]->karpeg_file;
			$data['raw'] = pathinfo($ffl, PATHINFO_EXTENSION);
			if($data['raw']=="jpg"){
				$data['thumb'] = "assets/media/karpeg/".$peg->nip_baru."/".$data['komponen']."/thumb_".$data['isi'][0]->karpeg_file;
			} else {
		...
		$this->load->view('karpeg_edok/ijin',$data);
	}
	function ijin_formedit(){
		$data['idd'] = $this->session->userdata('idd');//id_ibel
		$surat = $this->m_karpeg->ini_karpeg($data['idd']);
		$data['isi'] = json_decode($surat->ijin_pimpinan);
		$this->load->view('karpeg_edok/ijin_formedit',$data);
	}
	function ijin_edit_aksi(){
		$this->m_karpeg->ijin_edit($_POST);
		echo "sukses";
	}
	function ijin_upload(){
		...
		$surat = $this->m_karpeg->ini_karpeg($data['idd']);
		$data['isi'] = json_decode($surat->ijin_pimpinan);
		$data['row'] = $this->m_karpeg->cek_dokumen($data['idd'],$_POST['komponen']);
		$data['nip_baru'] = $_POST['id_pegawai'];
		
		foreach($data['row'] AS $key=>$val){
			$ffl = "assets/media/karpeg/".$data['nip_baru']."/".$data['komponen']."/".$val->karpeg_file;
			$raw = pathinfo($ffl, PATHINFO_EXTENSION);
			if($raw=="jpg"){
				@$data['row'][$key]->thumb = "assets/media/karpeg/".$data['nip_baru']."/".$data['komponen']."/thumb_".$val->karpeg_file;
			} else {
				@$data['row'][$key]->thumb = "assets/media/foto/default/doc.png";
			}
		}

		$this->load->view('karpeg_edok/ijin_upload',$data);
	}
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	function pasfoto(){
		$this->session->set_userdata('pegawai_info',$_POST['id_pegawai']);	//id_ibel
		$this->session->set_userdata('boleh','tidak');
		$data['isi'] = Modules::run("appbkpp/profile/pasfoto");
		$this->load->view('karpeg_edok/sk_cpns',$data);
	}
	function sk_cpns(){
		$this->session->set_userdata('pegawai_info',$_POST['id_pegawai']);	//id_ibel
		$this->session->set_userdata('boleh','tidak');
	...
	function saveupload(){
		if(strlen($_FILES['artikel_file']['name'])>0){
				...
				if($ext=="jpg"){
						...
						//$config['thumb_marker']='';
						$config['source_image'] = 'assets/media/karpeg/'.$id_pegawai.'/'.$komponen.'/'.$result['raw'].".".$ext;
						$config['new_image'] = 'assets/media/karpeg/'.$id_pegawai.'/'.$komponen.'/thumb_'.$result['raw'].".".$ext;
...
////////////////////////////////
				...
				$nmF = $idd."_".$nmB.".".$ext;
				rename("assets/media/karpeg/".$id_pegawai."/".$komponen."/".$result['raw'].".".$ext,"assets/media/karpeg/".$id_pegawai."/".$komponen."/".$nmF);
				if($ext=="jpg"){
						rename("assets/media/karpeg/".$id_pegawai."/".$komponen."/thumb_".$result['raw'].".".$ext,"assets/media/karpeg/".$id_pegawai."/".$komponen."/thumb_".$nmF);
				}
				$this->m_karpeg->rename_dokumen($result['idd'],$nmF);
////////////////////////////////
	...
	function uploadFile($id_pegawai,$nama_file,$komponen,$idd){
		$this->load->helper('file');
			$path="assets/media/karpeg/".$id_pegawai."/";
			if(!file_exists($path)){	mkdir($path,755);	}
			$path2="assets/media/karpeg/".$id_pegawai."/".$komponen."/";
...
//		$config['max_size']	= '512';
		...
		if ( ! $this->upload->do_upload('artikel_file'))		{
			...
			$ttpp = $dn['raw_name'];
			$idB = $this->m_karpeg->simpan_dokumen($id_pegawai,$nama_file,$komponen,$idd);
			$data['raw'] = $ttpp;
			$data['status'] = "success";
			$data['idd'] = $idB;
//			$gg = Modules::run("appdok/pantau/kontrol",$id_pegawai);
	...
	function hapus_dok(){
		...
		$dok = $this->m_karpeg->ini_dokumen($id_dokumen);
		unlink("assets/media/karpeg/".$id_pegawai."/".$komponen."/".$dok->karpeg_file);
		unlink("assets/media/karpeg/".$id_pegawai."/".$komponen."/thumb_".$dok->karpeg_file);
		$hp = $this->m_karpeg->hapus_dokumen($id_dokumen,$id_pegawai,$komponen,$_POST['idd']);

//		$gg = Modules::run("appdok/pantau/kontrol",$id_pegawai);
			echo "<b>Sukses...</b>";
	}
	function edit_keterangan_aksi(){
		$this->m_karpeg->edit_keterangan_dokumen($_POST);
	...
	function zoom(){
		...
		$data['dokumen'] = $this->m_karpeg->cek_dokumen($data['idd'],$_POST['komponen']);
		foreach($data['dokumen'] AS $key=>$val){
			$ffl = "assets/media/karpeg/".$data['nip_baru']."/".$data['komponen']."/".$val->karpeg_file;
			@$data['dokumen'][$key]->row = pathinfo($ffl, PATHINFO_EXTENSION);
		}
		$this->load->view('karpeg_edok/zoom',$data);
	}
