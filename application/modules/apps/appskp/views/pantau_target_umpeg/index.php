								...
								<div class="col-lg-4">
									<div class="panel panel-default">
										<div class="panel-body">
											...
											<div class="form-group">
												<label>Tahapan Target SKP:</label>
													<select id="a_st_skp" name="a_st_skp" class="form-control" onchange="gridpagingA('end');">
														<option value="" <?=($pstskp=="")?"selected":"";?>>Semua...</option>
														<option value="BS" <?=($pstskp=="BS")?"selected":"";?>>Belum selesai</option>
														<?php
															foreach($st_skp as $key=>$val){
																$selPstrealisasi = ($key==$pstskp)?"selected":"";
...
<a href="<?=site_url();?>apptukin/pantau/refresh_dashboard/<?=$tahun;?>" target="_blank" id="tombol_dashboard">refresh DB</a>
</div></div>
...
var st_skp = $('#a_st_skp').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appskp/pantau_target_umpeg/getdata",
		data:{"hal": hal, "batas": batas,"cari":cari,"kode":kode,"pkt":pkt,"jbt":jbt,"ese":ese,"tahun":tahun,"st_skp":st_skp},
		beforeSend:function(){	
	...
	//tombol aksi<--
					...
					table = table+ "<div style='clear:both;margin-bottom:15px;'>";
					table = table+ "<div style='float:left; width:95px;'>Nama</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
