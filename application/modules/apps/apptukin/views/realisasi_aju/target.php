...
<div class="row">
	<div class="col-lg-12" style="text-align:right; padding-bottom:10px;">
		 <a href="<?=site_url('module/apptukin/realisasi_aju');?>"><button class="btn btn-primary" type="button"><i class="fa fa-fast-backward fa-fw"></i>Kembali...</button></a>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->
...
<div class="row">
	...
	<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenu1" data-toggle="dropdown"><span class="fa fa-tasks fa-fw"></span></button>
		<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenu1">
			<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm('form_kembalikan','1','1','1');"><i class="fa fa-download fa-fw"></i>Dikoreksi, kembalikan kepada Pegawai</a></li>
			<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm('form_acc','1','1','1');"><i class="fa fa-upload fa-fw"></i>Disetujui</a></li>
...
?>
<tr id='row_tugas_pokok_<?=$val->id_target;?>' <?=(!empty($val->rubah))?"class=\"danger\"":"";?>>
<td id='nomor_tugas_pokok_<?=$val->id_target;?>'><?=$no;?></td>
<td align=center>
<?php
if(!empty($val->rubah)){
	echo "<div class=\"btn btn-danger btn-xs\" title=\"Lihat Catatan Perubahan Target\" onclick=\"setForm('rubah_lihat',".$val->id_target.",".@$id_tpp.",'r');\"><i class=\"fa fa-warning fa-fw\"></i></div>";
} else {
...
function setForm(tujuan,idd,no,urutan){
	var kop = []; 
	kop['form_kembalikan'] = "FORM KOREKSI REALISASI SASARAN KINERJA PEGAWAI"; 
	kop['form_acc'] = "FORM PERSETUJUAN REALISASI SASARAN KINERJA PEGAWAI"; 
	...
	var act = []; 
	act['form_kembalikan'] = "<?=site_url();?>apptukin/realisasi_aju/kembalikan_aksi"; 
	act['form_acc'] = "<?=site_url();?>apptukin/realisasi_aju/acc_aksi"; 
	act['track'] = ""; 
	act['tambah_catatan'] = "<?=site_url();?>apptukin/realisasi_aju/tambah_catatan_aksi"; 
	act['edit_catatan'] = "<?=site_url();?>apptukin/realisasi_aju/edit_catatan_aksi"; 
	act['hapus_catatan'] = "<?=site_url();?>apptukin/realisasi_aju/hapus_catatan_aksi"; 
	act['rubah_lihat'] = ""; 
	var btt = []; 
	...
	btt['form_acc'] = "<button id='btAct' type=sumbit class='btn btn-primary' onclick='ajukan(); return false;'><i class='fa fa-upload fa-fw'></i> Setuju</button>"; 
	btt['track'] = ""; 
