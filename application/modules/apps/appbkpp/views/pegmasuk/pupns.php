...
<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">Registrasi PUPNS</h3>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->

<form role="form" id="form_master" action="<?=site_url();?>appbkpp/profile/pupns_<?=(isset($isi->id_pupns))?"edit":"input";?>_aksi">
	...
	<div class="panel-body">
		<div class="row">
			<div class="col-lg-3">
				...
				<input name="pupns_nomor" id="pupns_nomor" type=text class="form-control" value="<?=(isset($isi->pupns_nomor))?$isi->pupns_nomor:"";?>" disabled>
			</div>
			<!--//col-lg-3-->
			<div class="col-lg-3">
				<label>Tanggal Registrasi</label>
				<input name="pupns_tanggal" id="pupns_tanggal" type=text class="form-control" value="<?=(isset($isi->pupns_tanggal))?date("d-m-Y", strtotime($isi->pupns_tanggal)):"";?>" placeholder="dd-mm-YYY" disabled>
			</div>
			<!--//col-lg-3-->
...
<div class="col-lg-3">
	<div class="panel panel-info">
		<div class="panel-heading"><i class="fa fa-image fa-fw"></i> Dok. Reg. PUPNS</div>
		<div class="panel-body">
								<div class="thumbnail">
									<div class="caption" style="text-align:center;">
										<p>
										<a href="" class="label label-info" onclick="viewUppl('pupns','<?=$isi->id_pupns;?>');return false;"><i class="fa fa-upload fa-fw"></i> Atur Dokumen</a>
										<a href="" class="label label-default" onclick="zoom_dok('pupns','<?=$isi->id_pupns;?>');return false;"><i class="fa fa-search fa-fw"></i> Zoom</a>
										</p>
									</div>
...
<div id="komponen_temp" style="display:none;">pupns</div>
...
function simpan(){
		var data="";
		var dati="";
				var nama = $.trim($("#karpeg_nomor").val());
				var nipb = $.trim($("#karpeg_tanggal").val());
				data=data+""+nama+"**";
				if( nama ==""){	dati=dati+"NOMOR KARTU PEGAWAI tidak boleh kosong\n";	}
				if( nipb ==""){	dati=dati+"TANGGAL KARTU PEGAWAI tidak boleh kosong\n";	}
		if( dati !=""){
			alert(dati);
			return false;
		} else { simpan_aksi();	}
...
function simpan(){
		var data="";
		var dati="";
				var nama = $.trim($("#pupns_nomor").val());
				var nipb = $.trim($("#pupns_tanggal").val());
				data=data+""+nama+"**";
				if( nama ==""){	dati=dati+"NOMOR REGISTRASI PUPNS tidak boleh kosong\n";	}
				if( nipb ==""){	dati=dati+"TANGGAL REGISTRASI PUPNS tidak boleh kosong\n";	}
		if( dati !=""){
			alert(dati);
			return false;
		} else { simpan_aksi();	}
...
function batal_isi(){
	location.href = '<?=site_url();?>module/appbkpp/pegmasuk/pupns';
}
