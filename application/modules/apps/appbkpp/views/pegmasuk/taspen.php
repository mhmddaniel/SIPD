...
<div class="row">
	<div class="col-lg-12">
		 <h3 class="page-header">TASPEN</h3>
	</div><!-- /.col-lg-12 -->
</div><!-- /.row -->

<form role="form" id="form_master" action="<?=site_url();?>appbkpp/profile/taspen_<?=(isset($isi->id_taspen))?"edit":"input";?>_aksi">
<div class="row" id="content_taspen">
<div class="col-lg-9">
<div class="panel panel-info">
	<div class="panel-heading">
		<i class="fa fa-edit fa-fw"></i> <b>Form TASPEN</b>
	...
	<div class="panel-body">
		<div class="row">
			<div class="col-lg-4">
				...
				<input name="taspen_nomor" id="taspen_nomor" type=text class="form-control" value="<?=(isset($isi->taspen_nomor))?$isi->taspen_nomor:"";?>" disabled>
			</div>
			<!--//col-lg-3-->
			<div class="col-lg-4">
				<label>Tanggal Kartu TASPEN</label>
				<input name="taspen_tanggal" id="taspen_tanggal" type=text class="form-control" value="<?=(isset($isi->taspen_tanggal))?date("d-m-Y", strtotime($isi->taspen_tanggal)):"";?>" placeholder="dd-mm-YYYY" disabled>
			</div>
			<!--//col-lg-3-->
			<div class="col-lg-4">
				<label>Pejabat penandatangan</label>
				<input name="taspen_pejabat" id="taspen_pejabat" type=text class="form-control" value="<?=(isset($isi->taspen_pejabat))?$isi->taspen_pejabat:"";?>" disabled>
			</div>
			<!--//col-lg-3-->
		</div>
		<!--//row-->
...
<div class="col-lg-3">
	<div class="panel panel-info">
		<div class="panel-heading"><i class="fa fa-image fa-fw"></i> TASPEN</div>
		<div class="panel-body">
								<div class="thumbnail">
									<div class="caption" style="text-align:center;">
										<p>
										<a href="" class="label label-info" onclick="viewUppl('taspen','<?=$isi->id_taspen;?>');return false;"><i class="fa fa-upload fa-fw"></i> Atur Dokumen</a>
										<a href="" class="label label-default" onclick="zoom_dok('taspen','<?=$isi->id_taspen;?>');return false;"><i class="fa fa-search fa-fw"></i> Zoom</a>
										</p>
									</div>
...
<div id="komponen_temp" style="display:none;">taspen</div>
...
function simpan(){
		var data="";
		var dati="";
				var nama = $.trim($("#taspen_nomor").val());
				var nipb = $.trim($("#taspen_tanggal").val());
				data=data+""+nama+"**";
				if( nama ==""){	dati=dati+"NOMOR KARTU TASPEN tidak boleh kosong\n";	}
				if( nipb ==""){	dati=dati+"TANGGAL KARTU TASPEN tidak boleh kosong\n";	}
		if( dati !=""){
			alert(dati);
			return false;
		} else { simpan_aksi();	}
...
function batal_isi(){
	location.href = '<?=site_url();?>module/appbkpp/pegmasuk/taspen';
}
