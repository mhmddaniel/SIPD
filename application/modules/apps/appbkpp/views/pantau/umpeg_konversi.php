...
<th style="width:450px;text-align:center; vertical-align:middle">KONVERSI NIP</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var kode = $('#a_kode_unor').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pantau/get_konversi",
	...
	//tombol aksi<--
					...
					var ts="";
					var itemm = item.seno;
						if(itemm.st_konversi_nip_nomor=="benar"){	var stt_konversi_nip_nomor = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_konversi_nip_nomor = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_konversi_nip_tanggal=="benar"){	var stt_konversi_nip_tanggal = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_konversi_nip_tanggal = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						...
						ts = ts+ '<span>'+itemm.konversi_nip_nomor+' '+stt_konversi_nip_nomor+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Tanggal SK</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.konversi_nip_tanggall+' '+stt_konversi_nip_tanggal+'</span>';
						ts = ts+ '</div>';
...
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="awal" value="konversi_nip">';
	var tab = tab + '<input type="hidden" name="asal" value="appbkpp/pantau/konversi">';
