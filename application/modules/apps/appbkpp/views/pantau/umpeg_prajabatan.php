...
<th style="width:450px;text-align:center; vertical-align:middle">DIKLAT PRAJABATAN</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var kode = $('#a_kode_unor').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pantau/get_prajabatan",
	...
	//tombol aksi<--
					...
					var ts="";
					var itemm = item.seno;
						if(itemm.st_nomor_sttpl=="benar"){	var stt_nomor_sttpl = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_nomor_sttpl = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_tanggal_sttpl=="benar"){	var stt_tanggal_sttpl = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_tanggal_sttpl = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_penyelenggara=="benar"){	var stt_penyelenggara = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_penyelenggara = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						...
						ts = ts+ '<div style="float:left; width:115px;">Nomor STTPL</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.nomor_sttpl+' '+stt_nomor_sttpl+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Tanggal STTPL</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.tanggal_sttpll+' '+stt_tanggal_sttpl+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Penyelenggara</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.penyelenggara+' '+stt_penyelenggara+'</span>';
						ts = ts+ '</div>';
...
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="awal" value="sertifikat_prajab">';
	var tab = tab + '<input type="hidden" name="asal" value="appbkpp/pantau/prajabatan">';
