...
<th style="width:450px;text-align:center; vertical-align:middle">PENGANGKATAN CALON PNS</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var kode = $('#a_kode_unor').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pantau/get_cpns",
	...
	//tombol aksi<--
					...
					var ts="";
					var itemm = item.seno;
						if(itemm.st_sk_cpns_nomor=="benar"){	var stt_sk_cpns_nomor = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_sk_cpns_nomor = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_sk_cpns_tgl=="benar"){	var stt_sk_cpns_tgl = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_sk_cpns_tgl = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_tmt_cpns=="benar"){	var stt_tmt_cpns = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_tmt_cpns = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						...
						ts = ts+ '<span>'+itemm.sk_cpns_nomor+' '+stt_sk_cpns_nomor+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Tanggal SK</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.sk_cpns_tgll+' '+stt_sk_cpns_tgl+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">TMT CPNS</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.tmt_cpnss+' '+stt_tmt_cpns+'</span>';
						ts = ts+ '</div>';
...
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="awal" value="sk_cpns">';
	var tab = tab + '<input type="hidden" name="asal" value="appbkpp/pantau/cpns">';
