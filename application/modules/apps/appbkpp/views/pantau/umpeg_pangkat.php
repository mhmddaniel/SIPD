...
<th style="width:450px;text-align:center; vertical-align:middle">RIWAYAT KEPANGKATAN</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var kode = $('#a_kode_unor').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pantau/get_pangkat",
	...
	//tombol aksi<--
					...
					$.each( item.seno, function(index, itemm){
						...
						if(itemm.st_tmt_golongan=="benar"){	var stt_tmt_golongan = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_tmt_golongan = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						...
						ts = ts+ '<div style="float:left; width:115px;">Pangkat / Gol.</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.nama_pangkat+' - <b>'+itemm.nama_golongan+'</b></span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						...
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.sk_tangga+' '+stt_sk_tanggal+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">TMT Pangkat</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.tangga+' '+stt_tmt_golongan+'</span>';
						ts = ts+ '</div>';
...
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="awal" value="sk_pangkat">';
	var tab = tab + '<input type="hidden" name="asal" value="appbkpp/pantau/pangkat">';
