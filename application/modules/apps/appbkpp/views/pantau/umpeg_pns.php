...
<th style="width:450px;text-align:center; vertical-align:middle">PENGANGKATAN PNS</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var kode = $('#a_kode_unor').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pantau/get_pns",
	...
	//tombol aksi<--
					...
					var ts="";
					var itemm = item.seno;
						if(itemm.st_sk_pns_nomor=="benar"){	var stt_sk_pns_nomor = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_sk_pns_nomor = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_sk_pns_tanggal=="benar"){	var stt_sk_pns_tanggal = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_sk_pns_tanggal = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_tmt_pns=="benar"){	var stt_tmt_pns = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_tmt_pns = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						...
						ts = ts+ '<span>'+itemm.sk_pns_nomor+' '+stt_sk_pns_nomor+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Tanggal SK</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.sk_pns_tanggall+' '+stt_sk_pns_tanggal+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">TMT PNS</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.tmt_pnss+' '+stt_tmt_pns+'</span>';
						ts = ts+ '</div>';
...
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="awal" value="sk_pns">';
	var tab = tab + '<input type="hidden" name="asal" value="appbkpp/pantau/pns">';
