								...
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-user fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											<li><a href="#" onClick="cetak(1);return false;"><i class="fa fa-print fa-fw"></i> Cetak Daftar</a></li>
										</ul>
										Daftar Pegawai
									</div>
								</div>
...
<th style="width:450px;text-align:center; vertical-align:middle">BIODATA PEGAWAI</th>
</tr>
</thead>
<tbody id=list>
</tbody>
...
var kode = $('#a_kode_unor').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pantau/get_biodata",
	...
	//tombol aksi<--
					...
					var ts="";
					var itemm = item.seno;
						if(itemm.st_tempat_lahir=="benar"){	var stt_tempat_lahir = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_tempat_lahir = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_tanggal_lahirr=="benar"){	var stt_tanggal_lahirr = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_tanggal_lahirr = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_agama=="benar"){	var stt_agama = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_agama = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_status_perkawinan=="benar"){	var stt_status_perkawinan = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_status_perkawinan = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						if(itemm.st_pasfoto=="benar"){	var stt_pasfoto = '<span class="btn btn-success btn-xs"><i class="fa fa-check fa-fw"></i></span>';	}	else	{	var stt_pasfoto = '<span class="btn btn-danger btn-xs"><i class="fa fa-close fa-fw"></i></span>';	}
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Tempat lahir</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.tempat_lahir+' '+stt_tempat_lahir+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Tanggal lahir</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.tanggal_lahirr+' '+stt_tanggal_lahirr+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">Agama</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.agama+' '+stt_agama+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both;">';
						ts = ts+ '<div style="float:left; width:115px;">St. perkawinan</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+itemm.status_perkawinan+' '+stt_status_perkawinan+'</span>';
						ts = ts+ '</div>';
						ts = ts+ '<div style="clear:both; margin-bottom:5px;">';
						ts = ts+ '<div style="float:left; width:115px;">Pasfoto</div>';
						ts = ts+ '<div style="float:left; width:10px;">:</div>';
						ts = ts+ '<span>'+stt_pasfoto+'</span>';
						ts = ts+ '</div>';
...
function cetak(hal){
	window.open("<?=site_url();?>appbkpp/xls_data_umpeg/index/"+hal,"_blank");
}
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="awal" value="pasfoto">';
	var tab = tab + '<input type="hidden" name="asal" value="appbkpp/pantau/biodata">';
