											...
											<div class="form-group">
												<label>Aksi operator:</label>
													<select id="a_aksi" name="a_aksi" class="form-control" onchange="gridpaging(1);">
														<option value="tambah" selected>Penambahan pegawai</option>															
														<option value="kurang">Pengurangan pegawai</option>															
													</select>
											</div>
										</div>
									</div>
...
var aksi = $('#a_aksi').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/ekindata/get_log_delta",
		data:{"bulan":bulan,"tahun":tahun,"hal": hal, "batas": batas,"cari":cari,"aksi":aksi},
		beforeSend:function(){	
	...
	//tombol aksi-->
					...
					table = table+ "</td>";
	//tombol aksi<--
					table = table+ "<td><b>"+item.tPensiun+item.tMeninggal+item.tKeluar+"</b><br>"+item.tipe_dokumen+"</td>";
