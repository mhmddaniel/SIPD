								...
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-list fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											...
											<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-toggle-off fa-fw"></i> Daftar Jabatan Kosong</a></li>
...
var tanggal = $('#iptTanggal').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/pejabat/getkosong",
...
//											pemangku = pemangku+'<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'hapus\',\''+item.id_unor +'\');"><i class="fa fa-close fa-fw"></i> Hapus data</a></li>';
	...
	//tombol aksi-->
					table = table+ "<td style='padding:3px;'><div id='kol_2_"+item.id_unor+"'>"+item.kode_unor+" - "+item.id_unor+"<br /><b>"+item.nomenklatur_jabatan+"</b><br />"+item.nama_ese+" - <u>pada</u> :<br />"+item.nomenklatur_pada+"</div></td>";
					table = table+ "<td style='padding:3px;'><div class='btn btn-warning btn-xs' onclick=\"detil2("+item.id_unor+",'appbkpp/pejabat/pemangku_riwayat');return false;\">Lihat pejabat sebelumnya</div>"+pemangku+"</td>";
					table = table+ "</tr>";
					no++;
				}); //endeach
					$('#list').html(table);
