<tr id="row_tt" class="success prinsip">
	...
	<td id="ipt_jabatan">
		<div>
			<div style="float:left; width:130px;">No. SK TKK</div>
		...
		<div style="clear:both;">
			...
			<div style="float:left; width:10px;">:</div>
			<div style="float:left; padding-bottom:10px;"><input type="text" class="form-control" id="sk_pejabat" name="sk_pejabat" value="<?=(!isset($val->sk_pejabat))?'':$val->sk_pejabat;?>" style="width:300px;height:20px;padding:1px 0px 0px 5px;"></div>
		</div>
		<div style="clear:both; border-top:dotted #666 1px; padding-top:10px;">
		...
		<div style="clear:both;">
			...
			<?=form_dropdown('tugas_tambahan',$this->dropdowns->tugas_tambahan(),(!isset($val->tugas_tambahan))?'':$val->tugas_tambahan,'class="form-control" style="height:20px;padding:1px 0px 0px 5px;"');?>
			</div>
		</div>
	</td>
</tr>
...
var tanggal = $('#tmt_jabatan').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/unor/getdata_pengelola_only",
		data:{"hal": hal, "batas": batas,"tanggal":tanggal,"cari":cari,"kehal":"pagingB","ese":"xx"},
		beforeSend:function(){	
...
$(document).on('click', '.btn.pilih',function(){
	...
	$('#div_jabatan').html('');
	$('#nama_jenis_jabatan').removeAttr("disabled").html('<option value="">Pilih...</option><option value="jfu">Jabatan Fungsional Umum</option><option value="jft-guru">Guru</option>');
});
function pil_nama(){
	$('.sub').remove();
	$('#div_jabatan').html('');
...
var kode_unor = $('#kode_unor').val();
	$.ajax({
		...
		data:{"hal": hal, "batas": batas,"cari":cari,"jenis":jenis,"kehal":"pagingC","tanggal":tanggal,"kode_unor":kode_unor},
		beforeSend:function(){	
