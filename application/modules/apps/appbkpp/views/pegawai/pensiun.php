...
<form id="form_pensiun" method="post" enctype="multipart/form-data">
...
<button class="btn btn-primary" type="button" <?php if($grup=="admin"){?>onclick="setSubForm('tambah','xx','xx');"<?php } ?>><i class="fa fa-plus fa-fw"></i> Tambah pegawai pensiun</button>
</td>
</tr>
<tbody>
</table>
...
var batas = $('#item_lengthB').val();
	$.ajax({
		...
		data:{"hal": hal, "batas": batas,"sub":"pensiun","cari":cari,"kehal":"pagingB"},
		beforeSend:function(){	
...
function setSubForm(aksi,idd,no){
	$('.btn.batal').click();
        ...
        success:function(data){
			$('#form_pensiun').attr('action','<?=site_url("appbkpp/pegawai/formsub_pensiun_");?>'+aksi+'_aksi');
			$('#brow_'+idd).hide();
...
function simpan_pensiun(){
	...
	if(idm){
		var hasil=validasi_pensiun();
		if (hasil!=false) {
				$.ajax({
					type:"POST",
					url: $("#form_pensiun").attr('action'),
					data: $("#form_pensiun").serialize(),
					beforeSend:function(){	
						$('.bt_simpan').remove();
					},
					success:function(data){
