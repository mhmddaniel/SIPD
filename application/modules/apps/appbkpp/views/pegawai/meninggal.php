	...
	<div class="col-lg-12">
		<div class="panel panel-success">
			<div class="panel-heading">
						<i class="fa fa-fire fa-fw"></i> <b>Daftar Pegawai Meninggal <?=$grup;?></b>
...
<form id="form_meninggal" method="post" enctype="multipart/form-data">
...
<button class="btn btn-primary" type="button" <?php if($grup=="admin"){?>onclick="setSubForm('tambah','xx','xx');"<?php } ?>><i class="fa fa-plus fa-fw"></i> Tambah pegawai meninggal</button>
</td>
</tr>
<tbody>
</table>
...
function setSubForm(aksi,idd,no){
	$('.btn.batal').click();
        ...
        success:function(data){
			$('#form_meninggal').attr('action','<?=site_url("appbkpp/pegawai/formsub_meninggal_");?>'+aksi+'_aksi');
			$('#brow_'+idd).hide();
...
function simpan_meninggal(){
	...
	if(idm){
		var hasil=validasi_meninggal();
		if (hasil!=false) {
				$.ajax({
					type:"POST",
					url: $("#form_meninggal").attr('action'),
					data: $("#form_meninggal").serialize(),
					beforeSend:function(){	
						$('.bt_simpan').remove();
					},
					success:function(data){
