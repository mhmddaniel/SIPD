						<div class="row" id="div_opsi_keluar" style="display:none; padding:15px 5px 0px 5px;"><div class="col-lg-12"><div class="panel panel-success" id="panel_filter"><div class="panel-heading">
						<div class="row">
								<div class="col-lg-4">
											<div class="form-group">
												<label>Unit kerja:</label>
													<select id="a_keluar_kode_unor" name="a_keluar_kode_unor" class="form-control" onchange="gridpaging_keluar('end');">
								...
								<div class="col-lg-4">
											<div class="form-group">
												<label>Jenis jabatan:</label>
													<select id="a_keluar_jabatan" name="a_keluar_jabatan" class="form-control" onchange="gridpaging_keluar('end');">
														<option value="" selected>Semua...</option>
														<?php
								...
								<div class="col-lg-4">
											<div class="form-group">
												<label>Pangkat / golongan:</label>
													<select id="a_keluar_pangkat" name="a_keluar_pangkat" class="form-control" onchange="gridpaging_keluar('end');">
														<option value="" selected>Semua...</option>
														<?php
...
<div class="row" style="padding:15px 5px 5px 5px;">
	<div class="col-lg-6" style="margin-bottom:5px;">
							...
							<select class="form-control input-sm" id="item_length_keluar" style="width:70px;" onchange="gridpaging_keluar('end')">
	...
	<div class="col-lg-6" style="margin-bottom:5px;">
                            <div class="input-group" style="width:240px; float:right; padding:0px 2px 0px 2px;">
                                <input id="caripaging_keluar" onchange="gridpaging_keluar('end')" type="text" class="form-control" placeholder="Masukkan kata kunci..." value="<?=$cari;?>">
				...
				<div class="row" style="padding:5px;">
					<div class="col-lg-12" style="margin-bottom:5px;">
							...
							<thead>
								<tr>
<th style="width:70px;text-align:center; vertical-align:middle">AKSI</th>
...
<th style="width:300px;text-align:center; vertical-align:middle">JABATAN TERAKHIR</th>
							...
							<tbody id="list_keluar"></tbody>
							</table>
							</div><!-- table-responsive --->
							<div id="paging_keluar"></div>
							<div id="paging_print_keluar" style="display:none;"></div>
					</div>
				</div>
<script type="text/javascript">
$(document).ready(function(){
	gridpaging_keluar('end');
});
function repaging_keluar(){
	$( "#paging_keluar .pagingframe div" ).addClass("btn btn-default");
	$( "#paging_keluar .pagingframe div" ).click(function() {
		var ini = $( this ).html();
		if(ini=="Prev" || ini=="Next"){	var inu=$(this).attr('data-hal');	} else {	var inu=$(this).html();	}
		if(!$(this).hasClass("active"))	{	gridpaging_keluar(inu);	}
	});
}
function gopaging_keluar(){
	$("#paging_keluar #inputpaging").change(function() {
		var ini = $( this ).val();
		gridpaging_keluar(ini);
	});
}
function gridpaging_keluar(hal){
	var bulan = $('#bulan_act').html();
	var tahun = $('#tahun_act').html();
	var cari = $('#caripaging_keluar').val();
	var batas = $('#item_length_keluar').val();
	var kode = $('#a_keluar_kode_unor').val();
	var pns = $('#a_pns').val();
	var pkt = $('#a_keluar_pangkat').val();
	var jbt = $('#a_keluar_jabatan').val();
	var ese = $('#a_ese').val();
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/dafpeg/getkeluar",
		...
		beforeSend:function(){	
			$('#list_keluar').html('<tr><td colspan=6><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></td></tr>');
			$('#paging_keluar').html('');
		},
	...
	//tombol aksi<--
					...
					$('#list_keluar').html(table);
					$('#paging_keluar').html(data.pager);
					repaging_keluar();gopaging_keluar();
var ini="";
for(i=0;i<data.seg_print;i++){
	...
	ini = ini + '<div onclick="cetak('+(i+1)+');"  class="btn btn-success btn-xs" style="margin-right:10px;margin-top:5px;">Hal. '+(i+1)+' (item no.'+jj+' - '+kk+')</div><br/>';
					...
					$('#paging_print_keluar').html(ini);

			} else {
				$('#list_keluar').html("<tr id=isi class=gridrow><td colspan=8 align=center><b>Tidak ada data</b></td></tr>");
				$('#paging_keluar').html("");
			} // end if
...
function cetak_excel(){
	var ini = $('#paging_print_keluar').html();
