						<div class="row" id="div_opsi_meninggal" style="display:none; padding:15px 5px 0px 5px;"><div class="col-lg-12"><div class="panel panel-success" id="panel_filter"><div class="panel-heading">
						<div class="row">
								<div class="col-lg-4">
											<div class="form-group">
												<label>Unit kerja:</label>
													<select id="a_meninggal_kode_unor" name="a_meninggal_kode_unor" class="form-control" onchange="gridpaging_meninggal('end');">
								...
								<div class="col-lg-4">
											<div class="form-group">
												<label>Jenis jabatan:</label>
													<select id="a_meninggal_jabatan" name="a_meninggal_jabatan" class="form-control" onchange="gridpaging_meninggal('end');">
														<option value="" selected>Semua...</option>
														<?php
								...
								<div class="col-lg-4">
											<div class="form-group">
												<label>Pangkat / golongan:</label>
													<select id="a_meninggal_pangkat" name="a_meninggal_pangkat" class="form-control" onchange="gridpaging_meninggal('end');">
														<option value="" selected>Semua...</option>
														<?php
...
<div class="row" style="padding:15px 5px 5px 5px;">
	<div class="col-lg-6" style="margin-bottom:5px;">
							...
							<select class="form-control input-sm" id="item_length_meninggal" style="width:70px;" onchange="gridpaging_meninggal('end')">
	...
	<div class="col-lg-6" style="margin-bottom:5px;">
                            <div class="input-group" style="width:240px; float:right; padding:0px 2px 0px 2px;">
                                <input id="caripaging_meninggal" onchange="gridpaging_meninggal('end')" type="text" class="form-control" placeholder="Masukkan kata kunci..." value="<?=$cari;?>">
				...
				<div class="row" style="padding:5px;">
					<div class="col-lg-12" style="margin-bottom:5px;">
							...
							<tbody id="list_meninggal"></tbody>
							</table>
							</div><!-- table-responsive --->
							<div id="paging_meninggal"></div>
							<div id="paging_print_meninggal" style="display:none;"></div>
					</div>
				</div>
<script type="text/javascript">
$(document).ready(function(){
	gridpaging_meninggal('end');
});
function repaging_meninggal(){
	$( "#paging_meninggal .pagingframe div" ).addClass("btn btn-default");
	$( "#paging_meninggal .pagingframe div" ).click(function() {
		var ini = $( this ).html();
		if(ini=="Prev" || ini=="Next"){	var inu=$(this).attr('data-hal');	} else {	var inu=$(this).html();	}
		if(!$(this).hasClass("active"))	{	gridpaging_meninggal(inu);	}
	});
}
function gopaging_meninggal(){
	$("#paging_meninggal #inputpaging").change(function() {
		var ini = $( this ).val();
		gridpaging_meninggal(ini);
	});
}
function gridpaging_meninggal(hal){
	var bulan = $('#bulan_act').html();
	var tahun = $('#tahun_act').html();
	var cari = $('#caripaging_meninggal').val();
	var batas = $('#item_length_meninggal').val();
	var kode = $('#a_meninggal_kode_unor').val();
	var pns = $('#a_pns').val();
	var pkt = $('#a_meninggal_pangkat').val();
	var jbt = $('#a_meninggal_jabatan').val();
	var ese = $('#a_ese').val();
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbkpp/dafpeg/getmeninggal",
		...
		beforeSend:function(){	
			$('#list_meninggal').html('<tr><td colspan=6><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></td></tr>');
			$('#paging_meninggal').html('');
		},
	...
	//tombol aksi<--
					...
					$('#list_meninggal').html(table);
					$('#paging_meninggal').html(data.pager);
					repaging_meninggal();gopaging_meninggal();
var ini="";
for(i=0;i<data.seg_print;i++){
	...
	ini = ini + '<div onclick="cetak('+(i+1)+');"  class="btn btn-success btn-xs" style="margin-right:10px;margin-top:5px;">Hal. '+(i+1)+' (item no.'+jj+' - '+kk+')</div><br/>';
					...
					$('#paging_print_meninggal').html(ini);

			} else {
				$('#list_meninggal').html("<tr id=isi class=gridrow><td colspan=8 align=center><b>Tidak ada data</b></td></tr>");
				$('#paging_meninggal').html("");
			} // end if
...
function cetak_excel(){
	var ini = $('#paging_print_meninggal').html();
...
function ppost(idd,act){
	var cari = $('#caripaging_meninggal').val();
	var batas = $('#item_length_meninggal').val();
	var hal=$("#paging_meninggal #inputpaging").val();
