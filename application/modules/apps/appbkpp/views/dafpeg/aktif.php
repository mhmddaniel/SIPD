											...
											<div class="form-group">
												<label>Pangkat / golongan:</label>
													<select id="a_pangkat" name="a_pangkat" class="form-control" onchange="gridpaging_aktif('end');">
														...
														?>
															<option value="11,12,13,14" <?=($ppkt=="11,12,13,14")?"selected":"";?>>Golongan I</option>
															<option value="21,22,23,24" <?=($ppkt=="21,22,23,24")?"selected":"";?>>Golongan II</option>
															<option value="31,32,33,34" <?=($ppkt=="31,32,33,34")?"selected":"";?>>Golongan III</option>
															<option value="41,42,43,44" <?=($ppkt=="41,42,43,44,45")?"selected":"";?>>Golongan IV</option>
													</select>
											</div>
											<div class="form-group">
...
function gridpaging_aktif(hal){
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						table = table+ "<div class='utmNo'>"+no+"</div>";
...
<!--						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="detil('+item.id_pegawai+',\'appbina/absensi/pegawai\',\'ya\');return false;"><i class="fa fa-clock-o fa-fw"></i> Rincian Absensi Pegawai</a></li>';-->
						...
						if(item.jab_type=="js"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="detil('+item.id_pegawai+',\'appevip/profile\',\'ya\');return false;"><i class="fa fa-cogs fa-fw"></i> Index Profesionalitas</a></li>';
						}
						table = table+ '<li role="presentation" class="divider">';
						...
						table = table+ '<li role="presentation"><a href="<?=site_url();?>appdok/donlot/buka/'+item.id_pegawai+'" role="menuitem" tabindex="-1" target="_blank" style="cursor:pointer;"><i class="fa fa-folder fa-fw"></i> Buka Berkas</a></li>';
	...
	//tombol aksi<--
					...
					if(item.tugas_tambahan=='xx' || item.tugas_tambahan=='') {
						if(item.jab_type=="jfu"){
							table = table+ "<td style='padding:3px;'>" +item.nomenklatur_jabatan+"<br/><u>pada</u><br/>"+item.nama_unor+" :: "+item.nomenklatur_pada+"<br/><u>sejak</u>:<br> "+item.tmt_jabatan+"</td>";
						} else {
							table = table+ "<td style='padding:3px;'>" +item.nomenklatur_jabatan+"<br/><u>pada</u><br/>"+item.nomenklatur_pada+"<br/><u>sejak</u>:<br> "+item.tmt_jabatan+"</td>";
						}
					} else {
...
for(i=0;i<data.seg_print;i++){
	...
	ini = ini + '<div onclick="cetak('+(i+1)+');"  class="btn btn-success btn-xs" style="margin-right:10px;margin-top:5px;">Hal. '+(i+1)+' (item no.'+jj+' - '+kk+')</div><br/>';
			...
			if(data.utmAct=="ya"){	$('.utm').show();$('.utmNo').hide();	} else {	$('.utm').hide();$('.utmNo').show();	}
