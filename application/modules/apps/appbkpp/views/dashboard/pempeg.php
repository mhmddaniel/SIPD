            <div class="row">
                <div class="col-lg-12">
                    <h3 class="page-header">Dashboard Absensi
							<div class="btn-group pull-right">
								...
								<div class="btn btn-default" onclick="maju('<?=$hari_maju->tanggal_harian;?>');"u><i class="fa fa-forward fa-fw"></i></div>
							</div>				
					</h3>
                </div><!-- /.col-lg-12 -->
            </div><!-- /.row -->
...
<?php if(date('H-i:s') <= "11:00:00"){/*	echo "<meta http-equiv=\"refresh\" content=\"10\" />";*/	}	?>
<?php	if($tanggal==date('Y-m-d')){	?>
<div class="pull-left" id="output">20</div>
<div style="display:none;" id="timer">1000</div>
<?php } ?>
...
$(document).ready(function(){
	...
	<?php	if($tanggal==date('Y-m-d') && date("H:i:s")<="11:00:00"){	?>
	waktu();
	<?php } ?>
});
...
function pilihB(kode,eselon,jbt,status){
	if(kode!='x'){	$('#i_kode').val(kode);	}
	if(eselon!='x'){	$('#i_ese').val(eselon);	}
	if(status!='x'){	$('#i_status').val(status);	}
	if(jbt!='x'){	$('#i_jbt').val(jbt);	}
	$('#sb_act').attr('action','<?=site_url();?>module/appbina/harian').removeAttr('target').submit();
}
function maju(iHari){
	$('#i_hari').val(iHari);
	$('#hari_act').attr('action','<?=site_url();?>module/appbkpp/dashboard/pempeg').submit();
}
...
function refresh_db(){
	$.ajax({
		...
		beforeSend:function(){	
			$('#bt_refresh').replaceWith('<p class="pull-right"><i class="fa fa-spinner fa-spin fa-1x"></i><p>');
		},
		success:function(data){
			location.reload();
		}, // end success
...
function waktu() { 
	var wtime = $("#timer").html();
	var ntime = parseInt(wtime);
	var waktu = $("#output").html();
	var detik = parseInt(waktu);
	detik = detik-1;
	$("#output").html(detik);

	if ( detik == 0 ){	refresh_db();	} else {	setTimeout("waktu()",ntime);	}
}
</script>
<style>
