...
///////////////////////////////////////////////////////////////////////////////////////
	...
	function ini_pegawai1($idd, $idc){
		$sqlstr="SELECT a.*,b.agama,b.gender,c.*,d.*,e.* FROM r_pegawai_aktual a LEFT JOIN r_pegawai b ON (a.id_pegawai=b.id_pegawai) LEFT JOIN r_peg_cuti_aju c ON (a.id_pegawai=c.id_pegawai) LEFT JOIN r_peg_cuti d ON (c.id_peg_cuti=d.id_peg_cuti) LEFT JOIN r_agenda e ON (c.id_agenda=e.id_agenda) WHERE a.id_pegawai='$idd' AND c.id_peg_cuti='$idc'";
		$hslquery=$this->db->query($sqlstr)->row();
		...
		@$hslquery->nama_user = $sess['nama_user'];
...
///////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function alamat_edit_aksi($id_pegawai,$data)	{
		...
		$this->db->set('ktp_nomor',$data['ktp_nomor']);
		$this->db->where('id_pegawai',$id_pegawai);
		$result = $this->db->update('r_peg_alamat');


		/////////////////// log>>>>>>>>
		$nLama = '{"jalan":"'.$alamat->jalan.'","rt":"'.$alamat->rt.'","rw":"'.$alamat->rw.'","kel_desa":"'.$alamat->kel_desa.'","kecamatan":"'.$alamat->kecamatan.'","kab_kota":"'.$alamat->kab_kota.'","propinsi":"'.$alamat->propinsi.'","ktp_nomor":"'.$alamat->ktp_nomor.'"}';
		$nBaru = '{"jalan":"'.$data['jalan'].'","rt":"'.$data['rt'].'","rw":"'.$data['rw'].'","kel_desa":"'.$data['kel_desa'].'","kecamatan":"'.$data['kecamatan'].'","kab_kota":"'.$data['kab_kota'].'","propinsi":"'.$data['propinsi'].'","ktp_nomor":"'.$data['ktp_nomor'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$alamat->id_peg_alamat);
		$this->db->set('tipe_dokumen','r_peg_alamat');
		$this->db->set('nip_baru',$id_pegawai);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
		$this->db->set('sub_keterangan',$nBaru);
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function get_npwp($idd){
		$this->db->from('r_peg_npwp');
		$this->db->where('id_pegawai',$idd);
		$hslquery = $this->db->get()->row();
		return $hslquery;
	}
    function ini_npwp($idd){
		$this->db->from('r_peg_npwp');
		$this->db->where('id_npwp',$idd);
		$hslquery = $this->db->get()->row();
		return $hslquery;
	}
    function npwp_input_aksi($isi){
		$this->db->set('npwp_nomor',$isi['npwp_nomor']);
		$this->db->set('npwp_tanggal',$isi['npwp_tanggal']);
		$this->db->set('npwp_pejabat',$isi['npwp_pejabat']);
		$this->db->set('id_pegawai',$isi['id_pegawai']);
		$this->db->insert('r_peg_npwp');
	}
    function npwp_edit_aksi($isi){
		$this->db->set('npwp_nomor',$isi['npwp_nomor']);
		$this->db->set('npwp_tanggal',$isi['npwp_tanggal']);
		$this->db->set('npwp_pejabat',$isi['npwp_pejabat']);
		$this->db->where('id_pegawai',$isi['id_pegawai']);
		$this->db->update('r_peg_npwp');
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function sertifikat_prajab_input_aksi($isi){
		...
		$id_peg_pend = $this->db_insert_id();

		/////////////////// log>>>>>>>>
		$nBaru = '{"id_rumpun":"5","nama_diklat":"'.$isi['nama_diklat'].'","tempat_diklat":"'.$isi['tempat_diklat'].',"penyelenggara":"'.$isi['penyelenggara'].'","tahun":"'.$isi['tahun'].'","angkatan":"'.$isi['angkatan'].'","tmt_diklat":"'.$isi['tmt_diklat'].'","tst_diklat":"'.$isi['tst_diklat'].'","jam":"'.$isi['jam'].'","nomor_sttpl":"'.$isi['nomor_sttpl'].'","tanggal_sttpl":"'.$isi['tanggal_sttpl'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$isi['id_pegawai']);
		$this->db->set('tipe_dokumen','r_peg_sertifikat_prajab');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
		$this->db->set('user_id',$sess['id_user']);
		$this->db->set('log_aksi',"NOW()",false);
		$this->db->insert('r_peg_dokumen');
    ...
    function sertifikat_prajab_edit_aksi($isi){
		$inipend = $this->get_sertifikat_prajab($isi['id_pegawai']);
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"id_rumpun":"5","nama_diklat":"'.$inipend->nama_diklat.'","tempat_diklat":"'.$inipend->tempat_diklat.',"penyelenggara":"'.$inipend->penyelenggara.'","tahun":"'.$inipend->tahun.'","angkatan":"'.$inipend->angkatan.'","tmt_diklat":"'.$inipend->tmt_diklat.'","tst_diklat":"'.$inipend->tst_diklat.'","jam":"'.$inipend->jam.'","nomor_sttpl":"'.$inipend->nomor_sttpl.'","tanggal_sttpl":"'.$inipend->tanggal_sttpl.'"}';
		$nBaru = '{"id_rumpun":"5","nama_diklat":"'.$isi['nama_diklat'].'","tempat_diklat":"'.$isi['tempat_diklat'].',"penyelenggara":"'.$isi['penyelenggara'].'","tahun":"'.$isi['tahun'].'","angkatan":"'.$isi['angkatan'].'","tmt_diklat":"'.$isi['tmt_diklat'].'","tst_diklat":"'.$isi['tst_diklat'].'","jam":"'.$isi['jam'].'","nomor_sttpl":"'.$isi['nomor_sttpl'].'","tanggal_sttpl":"'.$isi['tanggal_sttpl'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$isi['id_pegawai']);
		$this->db->set('tipe_dokumen','r_peg_sertifikat_prajab');
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function sk_cpns_input_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"tmt_cpns":"'.$isi['tmt_cpns'].'","sk_cpns_nomor":"'.$isi['sk_cpns_nomor'].'","sk_cpns_tgl":"'.$isi['sk_cpns_tgl'].'","sk_cpns_pejabat":"'.$isi['sk_cpns_pejabat'].'","mk_th":"'.$isi['mk_th'].'","mk_bl":"'.$isi['mk_bl'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_cpns');
		$this->db->set('id_reff',$isi['id_pegawai']);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
    ...
    function sk_cpns_edit_aksi($isi){
		$Inipend = $this->ini_cpns($isi['id_pegawai'] );
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"tmt_cpns":"'.$Inipend->tmt_cpns.'","sk_cpns_nomor":"'.$Inipend->sk_cpns_nomor.'","sk_cpns_tgl":"'.$Inipend->sk_cpns_tgl.'","sk_cpns_pejabat":"'.$Inipend->sk_cpns_pejabat.'","mk_th":"'.$Inipend->mk_th.'","mk_bl":"'.$Inipend->mk_bl.'"}';
		$nBaru = '{"tmt_cpns":"'.$isi['tmt_cpns'].'","sk_cpns_nomor":"'.$isi['sk_cpns_nomor'].'","sk_cpns_tgl":"'.$isi['sk_cpns_tgl'].'","sk_cpns_pejabat":"'.$isi['sk_cpns_pejabat'].'","mk_th":"'.$isi['mk_th'].'","mk_bl":"'.$isi['mk_bl'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_cpns');
		$this->db->set('id_reff',$isi['id_pegawai']);
...
///////////////////////////////////////////////////////////////////////////////////////
    ...
    function sk_pns_input_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"tmt_pns":"'.$isi['tmt_pns'].'","sk_pns_nomor":"'.$isi['sk_pns_nomor'].'","sk_pns_tanggal":"'.$isi['sk_pns_tanggal'].'","sk_pns_pejabat":"'.$isi['sk_pns_pejabat'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_pns');
		$this->db->set('id_reff',$isi['id_pegawai']);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
    ...
    function sk_pns_edit_aksi($isi){
		$Inipend = $this->ini_pns($isi['id_pegawai'] );
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"tmt_pns":"'.$Inipend->tmt_pns.'","sk_pns_nomor":"'.$Inipend->sk_pns_nomor.'","sk_pns_tanggal":"'.$Inipend->sk_pns_tanggal.'","sk_pns_pejabat":"'.$Inipend->sk_pns_pejabat.'"}';
		$nBaru = '{"tmt_pns":"'.$isi['tmt_pns'].'","sk_pns_nomor":"'.$isi['sk_pns_nomor'].'","sk_pns_tanggal":"'.$isi['sk_pns_tanggal'].'","sk_pns_pejabat":"'.$isi['sk_pns_pejabat'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_pns');
		$this->db->set('id_reff',$isi['id_pegawai']);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function pendidikan_riwayat_tambah_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"id_pendidikan":"'.$isi['id_pendidikan'].'","kode_jenjang":"'.$isi['kode_jenjang'].'","nama_jenjang":"'.$isi['nama_jenjang'].'","nama_pendidikan":"'.$isi['nama_pendidikan'].'","nama_jenjang_rumpun":"'.$isi['nama_jenjang_rumpun'].'","nama_sekolah":"'.$isi['nama_sekolah'].'","lokasi_sekolah":"'.$isi['lokasi_sekolah'].'","tanggal_lulus":"'.$isi['tanggal_lulus'].'","nomor_ijazah":"'.$isi['nomor_ijazah'].'","gelar_depan":"'.$isi['gelar_depan'].'","gelar_belakang":"'.$isi['gelar_belakang'].'","diakui":"'.$isi['diakui'].'","pendidikan_pertama":"'.$isi['pendidikan_pertama'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$id_peg_pend);
		$this->db->set('tipe_dokumen','r_peg_pendidikan');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
		$this->db->set('user_id',$sess['id_user']);
    ...
    function pendidikan_riwayat_edit_aksi($isi){
		$Inipend = $this->ini_riwayat_pendidikan($isi['id_peg_pendidikan']);
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"id_pendidikan":"'.$Inipend->id_pendidikan.'","kode_jenjang":"'.$Inipend->kode_jenjang.'","nama_jenjang":"'.$Inipend->nama_jenjang.'","nama_pendidikan":"'.$Inipend->nama_pendidikan.'","nama_jenjang_rumpun":"'.$Inipend->nama_jenjang_rumpun.'","nama_sekolah":"'.$Inipend->nama_sekolah.'","lokasi_sekolah":"'.$Inipend->lokasi_sekolah.'","tanggal_lulus":"'.$Inipend->tanggal_lulus.'","nomor_ijazah":"'.$Inipend->nomor_ijazah.'","gelar_depan":"'.$Inipend->gelar_depan.'","gelar_belakang":"'.$Inipend->gelar_belakang.'","diakui":"'.$Inipend->diakui.'","pendidikan_pertama":"'.$Inipend->pendidikan_pertama.'"}';
		$nBaru = '{"id_pendidikan":"'.$isi['id_pendidikan'].'","kode_jenjang":"'.$isi['kode_jenjang'].'","nama_jenjang":"'.$isi['nama_jenjang'].'","nama_pendidikan":"'.$isi['nama_pendidikan'].'","nama_jenjang_rumpun":"'.$isi['nama_jenjang_rumpun'].'","nama_sekolah":"'.$isi['nama_sekolah'].'","lokasi_sekolah":"'.$isi['lokasi_sekolah'].'","tanggal_lulus":"'.$isi['tanggal_lulus'].'","nomor_ijazah":"'.$isi['nomor_ijazah'].'","gelar_depan":"'.$isi['gelar_depan'].'","gelar_belakang":"'.$isi['gelar_belakang'].'","diakui":"'.$isi['diakui'].'","pendidikan_pertama":"'.$isi['pendidikan_pertama'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$isi['id_peg_pendidikan']);
		$this->db->set('tipe_dokumen','r_peg_pendidikan');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
    ...
    function pendidikan_riwayat_hapus_aksi($isi){
		...
		$this->db->set('tipe_dokumen','r_peg_pendidikan');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','hapus');
		$this->db->set('keterangan',$nLama);
		$this->db->set('user_id',$sess['id_user']);
		$this->db->set('log_aksi',"NOW()",false);
		$this->db->insert('r_peg_dokumen');
...
//////////////////////////////////////////////////////////////////////////////////
	...
	function ini_cuti_riwayat($idd){
		$this->db->from('r_peg_cuti');
		$this->db->where('id_peg_cuti',$idd);
		$hslquery = $this->db->get()->row();
		return $hslquery;
	}
    function pangkat_riwayat_tambah_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"tmt_golongan":"'.$isi['tmt_golongan'].'","kode_golongan":"'.$isi['kode_golongan'].'","nama_golongan":"'.$isi['nama_golongan'].'","nama_pangkat":"'.$isi['nama_pangkat'].'","sk_nomor":"'.$isi['sk_nomor'].'","sk_tanggal":"'.$isi['sk_tanggal'].'","bkn_nomor":"'.$isi['bkn_nomor'].'","bkn_tanggal":"'.$isi['bkn_tanggal'].'","kredit_utama":"'.$isi['kredit_utama'].'","kredit_tambahan":"'.$isi['kredit_tambahan'].'","mk_gol_tahun":"'.$isi['mk_gol_tahun'].'","mk_gol_bulan":"'.$isi['mk_gol_bulan'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('id_reff',$id_peg_golongan);
		$this->db->set('tipe_dokumen','r_peg_golongan');
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
		$this->db->set('user_id',$sess['id_user']);
		$this->db->set('log_aksi',"NOW()",false);
    ...
    function pangkat_riwayat_edit_aksi($isi){
		$Inipend = $this->ini_pangkat_riwayat($isi['id_peg_golongan']);
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"tmt_golongan":"'.$Inipend->tmt_golongan.'","kode_golongan":"'.$Inipend->kode_golongan.'","nama_golongan":"'.$Inipend->nama_golongan.'","nama_pangkat":"'.$Inipend->nama_pangkat.'","sk_nomor":"'.$Inipend->sk_nomor.'","sk_tanggal":"'.$Inipend->sk_tanggal.'","bkn_nomor":"'.$Inipend->bkn_nomor.'","bkn_tanggal":"'.$Inipend->bkn_tanggal.'","kredit_utama":"'.$Inipend->kredit_utama.'","kredit_tambahan":"'.$Inipend->kredit_tambahan.'","mk_gol_tahun":"'.$Inipend->mk_gol_tahun.'","mk_gol_bulan":"'.$Inipend->mk_gol_bulan.'"}';
		$nBaru = '{"tmt_golongan":"'.$isi['tmt_golongan'].'","kode_golongan":"'.$isi['kode_golongan'].'","nama_golongan":"'.$isi['nama_golongan'].'","nama_pangkat":"'.$isi['nama_pangkat'].'","sk_nomor":"'.$isi['sk_nomor'].'","sk_tanggal":"'.$isi['sk_tanggal'].'","bkn_nomor":"'.$isi['bkn_nomor'].'","bkn_tanggal":"'.$isi['bkn_tanggal'].'","kredit_utama":"'.$isi['kredit_utama'].'","kredit_tambahan":"'.$isi['kredit_tambahan'].'","mk_gol_tahun":"'.$isi['mk_gol_tahun'].'","mk_gol_bulan":"'.$isi['mk_gol_bulan'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('id_reff',$isi['id_peg_golongan']);
		$this->db->set('tipe_dokumen','r_peg_golongan');
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
		$this->db->set('sub_keterangan',$nBaru);
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function jabatan_riwayat_tambah_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"id_unor":"'.$isi['id_unor'].'","kode_unor":"'.$isi['kode_unor'].'","nama_unor":"'.$isi['nama_unor'].'","nama_jabatan":"'.$isi['nama_jabatan'].'","nama_jenis_jabatan":"'.$isi['nama_jenis_jabatan'].'","tmt_jabatan":"'.$isi['tmt_jabatan'].'","nomenklatur_pada":"'.$isi['nomenklatur_pada'].'","sk_nomor":"'.$isi['sk_nomor'].'","sk_tanggal":"'.$isi['sk_tanggal'].'","sk_pejabat":"'.$isi['sk_pejabat'].'"}';
		$this->db->set('id_reff',$id_peg_jab);
		$this->db->set('tipe_dokumen','r_peg_jab');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
		$this->db->set('user_id',$uid);
    ...
    function jabatan_riwayat_edit_aksi($isi){
		$pend = $this->ini_jabatan_riwayat($isi['id_peg_jab']);
		$sess = $this->session->userdata('logged_in');
		$uid = $sess['id_user'];
		foreach($isi AS $key=>$val){	$this->db->set($key,$val);	}
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"id_unor":"'.$pend->id_unor.'","kode_unor":"'.$pend->kode_unor.'","nama_unor":"'.$pend->nama_unor.'","nama_jabatan":"'.$pend->nama_jabatan.'","nama_jenis_jabatan":"'.$pend->nama_jenis_jabatan.'","tmt_jabatan":"'.$pend->tmt_jabatan.'","sk_nomor":"'.$pend->sk_nomor.'","sk_tanggal":"'.$pend->sk_tanggal.'","sk_pejabat":"'.$pend->sk_pejabat.'","tugas_tambahan":"'.$pend->tugas_tambahan.'","nomenklatur_pada":"'.$pend->nomenklatur_pada.'"}';
		$nBaru = '{"id_unor":"'.$isi['id_unor'].'","kode_unor":"'.$isi['kode_unor'].'","nama_unor":"'.$isi['nama_unor'].'","nama_jabatan":"'.$isi['nama_jabatan'].'","nama_jenis_jabatan":"'.$isi['nama_jenis_jabatan'].'","tmt_jabatan":"'.$isi['tmt_jabatan'].'","sk_nomor":"'.$isi['sk_nomor'].'","sk_tanggal":"'.$isi['sk_tanggal'].'","sk_pejabat":"'.$isi['sk_pejabat'].'","tugas_tambahan":"'.$isi['tugas_tambahan'].'","nomenklatur_pada":"'.$isi['nomenklatur_pada'].'"}';
		$this->db->set('id_reff',$isi['id_peg_jab']);
		$this->db->set('tipe_dokumen','r_peg_jab');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function penghargaan_tambah_aksi($isi){
		...
		$iIdPak = $this->db->insert_id();

		/////////////////// log>>>>>>>>
		$nBaru = '{"nama_penghargaan":"'.$isi['nama_penghargaan'].'","tempat_penghargaan":"'.$isi['tempat_penghargaan'].'","penyelenggara":"'.$isi['penyelenggara'].'","tahun":"'.$isi['tahun'].'","angkatan":"'.$isi['angkatan'].'","nomor_sertifikat":"'.$isi['nomor_sertifikat'].'","tanggal_sertifikat":"'.$isi['tanggal_sertifikat'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_penghargaan');
		$this->db->set('id_reff',$iIdPak);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
		$this->db->set('user_id',$sess['id_user']);
    ...
    function penghargaan_edit_aksi($isi){
		$inip = $this->riwayat_sertifikat_penghargaan($isi['id_peg_penghargaan']);
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"nama_penghargaan":"'.$inip->nama_penghargaan.'","tempat_penghargaan":"'.$inip->tempat_penghargaan.'","penyelenggara":"'.$inip->penyelenggara.'","tahun":"'.$inip->tahun.'","angkatan":"'.$inip->angkatan.'","nomor_sertifikat":"'.$inip->nomor_sertifikat.'","tanggal_sertifikat":"'.$inip->tanggal_sertifikat.'"}';
		$nBaru = '{"nama_penghargaan":"'.$isi['nama_penghargaan'].'","tempat_penghargaan":"'.$isi['tempat_penghargaan'].'","penyelenggara":"'.$isi['penyelenggara'].'","tahun":"'.$isi['tahun'].'","angkatan":"'.$isi['angkatan'].'","nomor_sertifikat":"'.$isi['nomor_sertifikat'].'","tanggal_sertifikat":"'.$isi['tanggal_sertifikat'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_penghargaan');
		$this->db->set('id_reff',$isi['id_peg_penghargaan']);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
		$this->db->set('sub_keterangan',$nBaru);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    function get_pak($idd){
		$sqlstr="SELECT a.*	FROM r_peg_pak a WHERE  a.id_pegawai = '$idd' ORDER BY a.tahun ASC, a.bulan ASC";
		$hslquery=$this->db->query($sqlstr)->result();
		return $hslquery;
	}
    ...
    function pak_tambah_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"penilai_nama_pegawai":"'.$isi['penilai_nama_pegawai'].'","ak":"'.$isi['ak'].'","ak_kumulatif":"'.$isi['ak_kumulatif'].'","tahun":"'.$isi['tahun'].'","bulan":"'.$bulan.'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_pak');
		$this->db->set('id_reff',$iIdPak);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
    ...
    function pak_edit_aksi($isi){
		$Inipend = $this->ini_pak($isi['id_pak']);
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"penilai_nama_pegawai":"'.$Inipend->penilai_nama_pegawai.'","ak":"'.$Inipend->ak.'","ak_kumulatif":"'.$Inipend->ak_kumulatif.'","tahun":"'.$Inipend->tahun.'","bulan":"'.$Inipend->bulan.'"}';
		$nBaru = '{"penilai_nama_pegawai":"'.$isi['penilai_nama_pegawai'].'","ak":"'.$isi['ak'].'","ak_kumulatif":"'.$isi['ak_kumulatif'].'","tahun":"'.$isi['tahun'].'","bulan":"'.$bulan.'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_pak');
		$this->db->set('id_reff',$isi['id_pak']);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
		$this->db->set('sub_keterangan',$nBaru);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function kgb_tambah_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"kode_golongan":"'.$isi['kode_golongan'].'","mk_gol_tahun":"'.$isi['mk_gol_tahun'].'","mk_gol_bulan":"'.$isi['mk_gol_bulan'].'","no_sk":"'.$isi['no_sk'].'","tanggal_sk":"'.$isi['tanggal_sk'].'","gaji_lama":"'.$gaji_lama.'","gaji_baru":"'.$gaji_baru.'","tmt_gaji":"'.$tmt_gaji.'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_kgb');
		$this->db->set('id_reff',$iIdPak);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
    ...
    function kgb_edit_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nLama = '{"kode_golongan":"'.$inip->kode_golongan.'","mk_gol_tahun":"'.$inip->mk_gol_tahun.'","mk_gol_bulan":"'.$inip->mk_gol_bulan.'","no_sk":"'.$inip->no_sk.'","tanggal_sk":"'.$inip->tanggal_sk.'","gaji_lama":"'.$inip->gaji_lama.'","gaji_baru":"'.$inip->gaji_baru.'","tmt_gaji":"'.$inip->tmt_gaji.'"}';
		$nBaru = '{"kode_golongan":"'.$isi['kode_golongan'].'","mk_gol_tahun":"'.$isi['mk_gol_tahun'].'","mk_gol_bulan":"'.$isi['mk_gol_bulan'].'","no_sk":"'.$isi['no_sk'].'","tanggal_sk":"'.$isi['tanggal_sk'].'","gaji_lama":"'.$gaji_lama.'","gaji_baru":"'.$gaji_baru.'","tmt_gaji":"'.$tmt_gaji.'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('tipe_dokumen','r_peg_kgb');
		$this->db->set('id_reff',$isi['id_kgb']);
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
		$this->db->set('sub_keterangan',$nBaru);
...
//		$this->db->select('year(a.tmt_diklat) as tahun,a.*',false);
    ...
    function diklat_struk_riwayat_tambah_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$nBaru = '{"id_diklat":"'.$isi['id_diklat'].'","id_rumpun":"'.$isi['id_rumpun'].'","jenis_diklat":"'.$isi['jenis_diklat'].'","jenjang_diklat":"'.$isi['jenjang_diklat'].'","nama_diklat":"'.$isi['nama_diklat'].'","tempat_diklat":"'.$isi['tempat_diklat'].'","penyelenggara":"'.$isi['penyelenggara'].'","tahun":"'.$isi['tahun'].'","angkatan":"'.$isi['angkatan'].'","tmt_diklat":"'.$isi['tmt_diklat'].'","tst_diklat":"'.$isi['tst_diklat'].'","jam":"'.$isi['jam'].'","nomor_sttpl":"'.$isi['nomor_sttpl'].'","tanggal_sttpl":"'.$isi['tanggal_sttpl'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$id_peg_pend);
		$this->db->set('tipe_dokumen','r_peg_diklat_struk');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','tambah');
		$this->db->set('sub_keterangan',$nBaru);
		$this->db->set('user_id',$sess['id_user']);
    ...
    function diklat_struk_riwayat_edit_aksi($isi){
		...
		/////////////////// log>>>>>>>>
		$Inipend = $this->ini_diklat_struk_riwayat($isi['id_peg_diklat_struk']);
		$nLama = '{"id_diklat":"'.$Inipend->id_diklat.'","id_rumpun":"'.$Inipend->id_rumpun.'","jenis_diklat":"'.$Inipend->jenis_diklat.'","jenjang_diklat":"'.$Inipend->jenjang_diklat.'","nama_diklat":"'.$Inipend->nama_diklat.'","tempat_diklat":"'.$Inipend->tempat_diklat.'","penyelenggara":"'.$Inipend->penyelenggara.'","tahun":"'.$Inipend->tahun.'","angkatan":"'.$Inipend->angkatan.'","tmt_diklat":"'.$Inipend->tmt_diklat.'","tst_diklat":"'.$Inipend->tst_diklat.'","jam":"'.$Inipend->jam.'","nomor_sttpl":"'.$Inipend->nomor_sttpl.'","tanggal_sttpl":"'.$Inipend->tanggal_sttpl.'"}';
		$nBaru = '{"id_diklat":"'.$isi['id_diklat'].'","id_rumpun":"'.$isi['id_rumpun'].'","jenis_diklat":"'.$isi['jenis_diklat'].'","jenjang_diklat":"'.$isi['jenjang_diklat'].'","nama_diklat":"'.$isi['nama_diklat'].'","tempat_diklat":"'.$isi['tempat_diklat'].'","penyelenggara":"'.$isi['penyelenggara'].'","tahun":"'.$isi['tahun'].'","angkatan":"'.$isi['angkatan'].'","tmt_diklat":"'.$isi['tmt_diklat'].'","tst_diklat":"'.$isi['tst_diklat'].'","jam":"'.$isi['jam'].'","nomor_sttpl":"'.$isi['nomor_sttpl'].'","tanggal_sttpl":"'.$isi['tanggal_sttpl'].'"}';
		$sess = $this->session->userdata('logged_in');
		$this->db->set('id_reff',$isi['id_peg_diklat_struk']);
		$this->db->set('tipe_dokumen','r_peg_diklat_struk');
		$this->db->set('nip_baru',$isi['id_pegawai']);
		$this->db->set('file_dokumen','edit');
		$this->db->set('keterangan',$nLama);
