...
class Profile_berkala extends MX_Controller {
	...
	function __construct(){
		...
		$this->load->model('appbkpp/m_profil_berkala');
...
///////////////////////////////////////////////////////////////////////////////////
	...
	function alih_v2(){
		...
		redirect(site_url("admin/module/appbkpp/profile"));
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function ini_berkala($id_pegawai=false){
		$result = $this->m_profil_berkala->ini_berkala($id_pegawai);
		return $result;
	}

	function ini_berkala_lanjutan($id_pegawai=false,$mk_cpns_tahun=false,$mk_cpns_bulan=false){
		$result = $this->m_profil_berkala->ini_berkala_lanjutan($id_pegawai,$mk_cpns_tahun,$mk_cpns_bulan);
		return $result;
	}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	function utama(){
		...
		$data['data'] = $this->m_profil->ini_pegawai_master($id_pegawai);
		$sess = $this->session->userdata('logged_in');
		$data['editable'] = (($sess['group_name']=="admin") && $boleh=="ya")?"yes":"no";
...
////////////////////////////////////////////////////////////////////
	...
	function sk_jabatan(){
		...
		$boleh = $this->session->userdata('boleh');
		$sess = $this->session->userdata('logged_in');
		$editable = (($sess['group_name']=="admin" || $sess['group_name']=="pengelolaa" || $sess['group_name']=="mutasi" || $sess['group_name']=="opwebbkpp") && $boleh=="ya")?"yes":"no";
	...
	function jabatan_riwayat(){
		$jabatan = $this->m_profil->ini_pegawai_jabatan($_POST['id_pegawai']);
			$data['jabatan'] = '';
			$mulai=0;
			foreach($jabatan as $row){
				$row->no=$mulai+1;
				$row->tmt_jabatan = date("d-m-Y", strtotime($row->tmt_jabatan));
				$row->sk_tanggal = date("d-m-Y", strtotime($row->sk_tanggal));
				$dok_ref = $this->m_edok->cek_dokumen($row->nip_baru,"sk_jabatan",$row->id_peg_jab);
				$row->thumb = (empty($dok_ref))?"assets/file/foto/photo.jpg":"assets/media/file/".$row->nip_baru."/sk_jabatan/thumb_".$dok_ref[0]->file_dokumen;
				$data['jabatan'] .= $this->load->view('profile/formjabatan_row',array('val'=>$row),true);
				$mulai++;
			}
		$data['no']=$mulai+1;
