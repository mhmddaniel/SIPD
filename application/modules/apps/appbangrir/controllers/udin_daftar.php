...
class Udin_daftar extends MX_Controller {
...
///////////////////////////////////////////////////////////////////////////////////
  function tahapan_udin($asRef=false)  {
    $select ['buat'] = 'Pembuatan Pengajuan Ujian Dinas';
    $select ['draft'] = 'Pengisian Formulir Ujian Dinas';
    $select ['aju'] = 'Pengajuan Ujian Dinas';
    $select ['koreksi'] = 'Koreksi Pengajuan Ujian Dinas';
    $select ['revisi'] = 'Perbaikan Pengajuan Ujian Dinas';
    $select ['acc'] = 'Persetujuan Ujian Dinas';
    return $select;
  }
  function kode_dokumen_udin($asRef=false)  {
...
///////////////////////////////////////////////////////////////////////////////////
	function index(){
		...
		$this->load->view('udin_daftar/index',$data);
	}
	...
	function getdata(){
		...
		if($group_name=="pengelola"){
			...
			$dWtahapan = $this->tahapan_udin();
			$bulan = (strlen($_POST['bulan'])==1)?"0".$_POST['bulan']:$_POST['bulan'];
			$tahun=$_POST['tahun'];
			$data['count'] = $this->m_udin->hitung_udin($_POST['cari'],$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stpi);
		...
		if($data['count']==0){
			...
			$data['hslquery'] = $this->m_udin->get_udin($_POST['cari'],$mulai,$batas,$_POST['pns'],$unor,$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur,$mkcpns,$stpi);
			foreach($data['hslquery'] AS $key=>$val){
				...
				$accIb = $this->m_udin->ini_acc($val->id_udin);
				if(empty($accIb)){
					@$acc['nomor_surat'] = "...";
					@$acc['tanggal_surat'] = "...";
					@$acc['editable'] = "yes";
					...
					@$acc['editable'] = "yes";
					@$acc['thumb'] = $this->ini_thumb($val->nip_baru,$accIb->id_peg_ujian_dinas);
					@$acc['idd'] = $accIb->id_peg_ujian_dinas;
				}
	...
	function cek_dok($idd){
		$this->db->from('r_peg_ujian_dinas_dokumen');
		$this->db->where('id_udin',$idd);
		$data = $this->db->get()->result();
		$hps = (empty($data))?"ya":"tidak";
		return $hps;
	}
	function ini_thumb($nip,$idd){
		$gbr = $this->m_edok->cek_dokumen($nip,"ujian_dinas",$idd);
		$gambar = (empty($gbr))?"assets/file/foto/photo.jpg":"assets/media/file/".$nip."/ujian_dinas/thumb_".$gbr[0]->file_dokumen;
		return $gambar;
	}

	function form_tambah(){
		$this->load->view('udin_daftar/form_tambah');
	}
	function cari_nip(){
			$this->load->model('appbkpp/m_umpeg');
			$user_id = $this->session->userdata('user_id');
		...
		$this->load->view('udin_daftar/form_tambah_nip',$data);
	}
	function tambah_aksi(){
		$this->db->from('r_pegawai_aktual');
		$this->db->where('id_pegawai',$_POST['id_pegawai']);
		$peg = $this->db->get()->row();
		$id_udin = $this->m_udin->tambah_pemohon($peg);

			$_POST['id_udin'] = $id_udin;
			$this->m_udin->sekolah_tambah($_POST);
	...
	function form_hapus(){
			$dWpangkat = $this->dropdowns->kode_pangkat();
			$dWgolongan = $this->dropdowns->kode_golongan();
		$data['val'] = $this->m_udin->ini_udin($_POST['idd']);
		$data['val']->nama_pangkat = @$dWpangkat[$val->kode_golongan];
		$data['val']->nama_golongan = @$dWgolongan[$val->kode_golongan];
		$this->load->view('udin_daftar/form_hapus',$data);
	}
	function hapus_aksi(){
		$this->db->where('id_udin',$_POST['id_udin']);
		$this->db->delete('r_peg_ujian_dinas_aju');
		$this->db->where('id_udin',$_POST['id_udin']);
		$this->db->delete('r_peg_ujian_dinas_sekolah');
		echo "sukses";
	}
	function ajukan(){
		...
		$this->load->view('udin_daftar/form_ajukan',$data);
	}

	function ajukan_aksi(){
		$this->m_udin->ajukan_pemohon($_POST['id_udin']);
		echo "sukses";
	}
	...
	function form_sekolah(){
		...
		$data['val']->nama_pangkat = @$dWpangkat[$val->kode_golongan];
		$data['val']->nama_golongan = @$dWgolongan[$val->kode_golongan];
		$data['row'] = $this->m_udin->ini_sekolah($_POST['idd']);
		$this->load->view('ibel_daftar/form_sekolah',$data);
	}

	function sekolah_aksi(){
		if($_POST['id_udin_sekolah']==""){
			$this->m_udin->sekolah_tambah($_POST);
		} else {
			$this->m_udin->sekolah_edit($_POST);
		}
		echo "sukses";
	}
