...
<div class="row">
	...
	<div class="col-lg-6">
		<div class="panel panel-default">
			<div class="panel-heading"><i class="fa fa-institution fa-fw"></i> <b>Sekolah</b></div>
			<div class="panel-body">
								<div>
										<div style="float:left; width:90px;">Nama</div>
										<div style="float:left; width:5px;">:</div>
										<span><div style="display:table;"><?=$sek->nama_sekolah;?></div></span>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Lokasi</div>
										<div style="float:left; width:5px;">:</div>
										<span><div style="display:table;"><?=$sek->lokasi_sekolah;?></div></span>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Jenjang</div>
										<div style="float:left; width:5px;">:</div>
								...
								<div style="clear:both">
										<div style="float:left; width:90px;">Gelar</div>
										<div style="float:left; width:5px;">:</div>
										<span><div style="display:table;"><?=$sek->gelar_depan;?> - <?=$sek->gelar_belakang;?></div></span>
								</div>
			</div>
		</div><!-- /.panel -->
	</div><!-- /.col-lg-6 -->
...
?>
	<div class="panel panel-danger">
		<div class="panel-heading">
			<h4 class="panel-title"><a class="collapsed" href="#collapseOnejawaban" data-parent="#accordion" data-toggle="collapse" onclick="isi('jawaban','proses');" id="tbtg_catatan"><?=$no;?>. Catatan Pemroses</a></h4>
		</div>
	...
	<div class="panel panel-warning">
		<div class="panel-heading">
			<h4 class="panel-title"><a class="collapsed" href="#collapseOneajukan" data-parent="#accordion" data-toggle="collapse" onclick="isi('ajukan','daftar');" id="tbtg_catatan"><?=$no+1;?>. Ajukan Permohonan</a></h4>
		</div>
...
function uppl(bttn,stts,dokumen){	
		...
		new AjaxUpload(btnUpload, {
			action: '<?=site_url();?>appbangrir/tubel_edok/saveupload',
			name: 'artikel_file',
...
function ok_hapus(tipe,idd){
	...
	var id_dokumen = $('#head_no_'+tipe).attr("data-id_dokumen");
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_edok/hapus_dok",
...
function isi(dok,prs){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_"+prs+"/"+dok,
...
function viewForm(komponen){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_edok/"+komponen+"_formedit",
				data:{"id_pegawai":"<?=$id_pegawai;?>"},
				beforeSend:function(){	
					$('#accordion').hide();
					$('#formedok').show();
...
function viewUppl(komponen,idd){
	...
	var nip_baru = $('#nip_baru').html();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_edok/"+komponen+"_upload",
...
function satuket(idd){
	...
	var sub_keterangan = $('#sub_ket_dok_'+idd).val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_edok/edit_keterangan_aksi",
...
function zoom_dox(komponen,idd){
	var nip_baru = $('#nip_baru').html();
	$('#sb_act').attr('action','<?=site_url();?>appbangrir/tubel_edok/zoom').attr('target','_blank');
	var tab = '<input type="hidden" name="komponen" value="'+komponen+'">';
	var tab = tab + '<input type="hidden" name="nip_baru" value="'+nip_baru+'">';	
	$('#sb_act').html(tab).submit();
}
