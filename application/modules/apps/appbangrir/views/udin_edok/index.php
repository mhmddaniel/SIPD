...
function uppl(bttn,stts,dokumen){	
		...
		new AjaxUpload(btnUpload, {
			action: '<?=site_url();?>appbangrir/udin_edok/saveupload',
			name: 'artikel_file',
...
function ok_hapus(tipe,idd){
	...
	var id_dokumen = $('#head_no_'+tipe).attr("data-id_dokumen");
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/udin_edok/hapus_dok",
...
function isi(dok,prs){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/udin_"+prs+"/"+dok,
...
function viewForm(komponen){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/udin_edok/"+komponen+"_formedit",
				data:{"id_pegawai":"<?=$id_pegawai;?>"},
				beforeSend:function(){	
					$('#accordion').hide();
					$('#formedok').show();
...
function viewUppl(komponen,idd){
	...
	var nip_baru = $('#nip_baru').html();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/udin_edok/"+komponen+"_upload",
...
function satuket(idd){
	...
	var sub_keterangan = $('#sub_ket_dok_'+idd).val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/udin_edok/edit_keterangan_aksi",
...
function zoom_dox(komponen,idd){
	var nip_baru = $('#nip_baru').html();
	$('#sb_act').attr('action','<?=site_url();?>appbangrir/udin_edok/zoom').attr('target','_blank');
	var tab = '<input type="hidden" name="komponen" value="'+komponen+'">';
	var tab = tab + '<input type="hidden" name="nip_baru" value="'+nip_baru+'">';	
	$('#sb_act').html(tab).submit();
}
