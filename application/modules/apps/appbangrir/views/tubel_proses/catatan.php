			...
			<table class="table table-striped table-bordered table-hover">
				<thead id=gridhead>
					<tr height=20>
						...
						<?php if($tubel->status=="aju" || $tubel->status=="koreksi"){ ?>
...
function save_catatan(){
		...
		var catatan = $("input[name='catatan']").val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_proses/save_catatan",
...
function hapus(){
		var id_catatan = $("input[name='id_catatan']").val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_proses/hapus_catatan",
...
function edit_catatan(idd){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/tubel_proses/ini_catatan",
				data:{"id_catatan":idd},
				beforeSend:function(){	
