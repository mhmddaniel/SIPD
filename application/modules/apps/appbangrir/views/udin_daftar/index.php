										...
										<div class="panel-body">
											...
											<div class="form-group">
												<label>Tahapan pengajuan:</label>
													<select id="a_studin" name="a_studin" class="form-control" onchange="gridpagingA('end');">
														<option value="" selected>Semua...</option>
														<?php
															foreach($studin as $key=>$val){
																$selStudin = ($key==$pstudin)?"selected":"";
																if($key!=""){	echo '<option value="'.$key.'" '.$selStudin.'>'.$val.'</option>';	}
															}
														?>
														<option value="berjalan"  <?=($pstudin=="berjalan")?"selected":"";?>>Berjalan</option>
														<option value="selesai"  <?=($pstudin=="selesai")?"selected":"";?>>Selesai</option>
														<option value="injek"  <?=($pstudin=="injek")?"selected":"";?>>Injek</option>
													</select>
											</div>
										</div>
									</div>
...
function setForm(tujuan,idd,no){
	var kop = []; 
	kop['form_tambah'] = "FORM TAMBAH PEGAWAI PEMOHON UJIAN DINAS"; 
	kop['form_sekolah'] = "FORM UJIAN DINAS"; 
	kop['form_hapus'] = "FORM HAPUS PEGAWAI PEMOHON"; 
	var act = []; 
	act['form_tambah'] = "<?=site_url();?>appbangrir/udin_daftar/tambah_aksi"; 
	act['form_sekolah'] = "<?=site_url();?>appbangrir/udin_daftar/sekolah_aksi"; 
	act['form_hapus'] = "<?=site_url();?>appbangrir/udin_daftar/hapus_aksi"; 
	var btt = [];
	...
	btt['form_hapus'] = "<button id='btAct' type=sumbit class='btn btn-danger' onclick='ajukan(); return false;'><i class='fa fa-trash fa-fw'></i> Hapus</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appbangrir/udin_daftar/"+tujuan,
			data:{"idd": idd,"no": no },
			beforeSend:function(){	
				$("#pageKonten").hide();
...
function gridpagingA(hal){
	...
	var studin = $('#a_studin').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbangrir/udin_daftar/getdata",
		data:{"bulan":bulan,"tahun":tahun,"hal": hal, "batas": batas,"cari":cari,"pns":pns,"kode":kode,"pkt":pkt,"jbt":jbt,"ese":ese,"tugas":tugas,"gender":gender,"agama":agama,"status":status,"jenjang":jenjang,"umur":umur,"mkcpns":mkcpns,"studin":studin},
		beforeSend:function(){	
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_sekolah\','+item.id_udin+','+no+');return false;"><i class="fa fa-pencil fa-fw"></i> Edit data</a></li>';
						}
						if(item.hapus=="ya" && accIb.thumb=="assets/file/foto/photo.jpg"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="setForm(\'form_hapus\','+item.id_udin+','+no+');return false;"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						}
						if((item.hapus=="ya" && accIb.thumb=="assets/file/foto/photo.jpg") || item.status=="draft"){
						table = table+ '<li role="presentation" class="divider">';
						}
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_udin+');return false;"><i class="fa fa-search fa-fw"></i> Lihat detail</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
//					table = table+ "<td style='padding:3px;'><b>"+item.nama_pegawai+"</b> ("+item.gender+")<br/>"+item.tempat_lahir+", "+item.tanggal_lahir+"<br/>"+item.nip_baru+" / "+item.tmt_pns+"</td>";
					...
					table = table+ "</div></td>";
					var sek = item.sekolah;
					table = table+ "<td><div>";
					table = table+ "<div style='float:left; width:65px;'>Nama</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'><b>"+sek.nama_sekolah+"</b></div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Alamat</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					...
					table = table+ '<a href="#" class="label label-default" onclick="zoom_dok(\'ujian_dinas\','+accIb.idd+',\''+item.nip_baru+'\');return false;"><i class="fa fa-search fa-fw"></i> Zoom</a>';
										}
					table = table+ "</p>";
					table = table+ "</div>";
					...
					table = table+ "</div>";
									...
									table = table+ "<div>";
									table = table+ "<div style='float:left; width:135px;'>Tanggal STLUD</div>";
									table = table+ "<div style='float:left; width:10px;'>:</div>";
									table = table+ "<div style='float:left;'>"+accIb.tanggal_surat+"</div>";
									table = table+ "</div>";
									table = table+ "<div style='clear:both'>";
									table = table+ "<div style='float:left; width:135px;'>Nomor STLUD</div>";
									table = table+ "<div style='float:left; width:10px;'>:</div>";
...
function ppost(idd,act){
	...
	$('#sb_act').attr('action','<?=site_url();?>appbangrir/udin_edok/alih');
	...
	var tab = tab + '<input type="hidden" name="asal" value="appbangrir/udin_daftar">';
