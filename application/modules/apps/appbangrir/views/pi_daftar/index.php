										...
										<div class="panel-body">
											...
											<div class="form-group">
												<label>Tahapan pengajuan:</label>
													<select id="a_stpi" name="a_stpi" class="form-control" onchange="gridpagingA('end');">
														<option value="" selected>Semua...</option>
														<?php
															foreach($stpi as $key=>$val){
																$selStpi = ($key==$pstpi)?"selected":"";
																if($key!=""){	echo '<option value="'.$key.'" '.$selStpi.'>'.$val.'</option>';	}
															}
														?>
														<option value="berjalan"  <?=($pstpi=="berjalan")?"selected":"";?>>Berjalan</option>
														<option value="selesai"  <?=($pstpi=="selesai")?"selected":"";?>>Selesai</option>
														<option value="injek"  <?=($pstpi=="injek")?"selected":"";?>>Injek</option>
													</select>
											</div>
										</div>
									</div>
...
function setForm(tujuan,idd,no){
	var kop = []; 
	kop['form_tambah'] = "FORM TAMBAH PEGAWAI PEMOHON PENYESUAIAN IJAZAH"; 
	kop['form_sekolah'] = "FORM PENYESUAIAN IJAZAH"; 
	kop['form_hapus'] = "FORM HAPUS PEGAWAI PEMOHON"; 
	var act = []; 
	act['form_tambah'] = "<?=site_url();?>appbangrir/pi_daftar/tambah_aksi"; 
	act['form_sekolah'] = "<?=site_url();?>appbangrir/pi_daftar/sekolah_aksi"; 
	act['form_hapus'] = "<?=site_url();?>appbangrir/pi_daftar/hapus_aksi"; 
	var btt = [];
	...
	btt['form_sekolah'] = "<button id='btAct' type=sumbit class='btn btn-primary' onclick='ajukan(); return false;'><i class='fa fa-save fa-fw'></i> Simpan</button>"; 
	btt['form_hapus'] = "<button id='btAct' type=sumbit class='btn btn-danger' onclick='ajukan(); return false;'><i class='fa fa-trash fa-fw'></i> Hapus</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appbangrir/pi_daftar/"+tujuan,
			data:{"idd": idd,"no": no },
			beforeSend:function(){	
				$("#pageKonten").hide();
...
function gridpagingA(hal){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbangrir/pi_daftar/getdata",
		data:{"bulan":bulan,"tahun":tahun,"hal": hal, "batas": batas,"cari":cari,"pns":pns,"kode":kode,"pkt":pkt,"jbt":jbt,"ese":ese,"tugas":tugas,"gender":gender,"agama":agama,"status":status,"jenjang":jenjang,"umur":umur,"mkcpns":mkcpns,"stpi":stpi},
		beforeSend:function(){	
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_sekolah\','+item.id_pi+','+no+');return false;"><i class="fa fa-pencil fa-fw"></i> Edit data</a></li>';
						}
						if(item.hapus=="ya" && accIb.thumb=="assets/file/foto/photo.jpg"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="setForm(\'form_hapus\','+item.id_pi+','+no+');return false;"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						}
						if((item.hapus=="ya" && accIb.thumb=="assets/file/foto/photo.jpg") || item.status=="draft"){
						table = table+ '<li role="presentation" class="divider">';
						}
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_pi+');return false;"><i class="fa fa-search fa-fw"></i> Lihat detail</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
//					table = table+ "<td style='padding:3px;'><b>"+item.nama_pegawai+"</b> ("+item.gender+")<br/>"+item.tempat_lahir+", "+item.tanggal_lahir+"<br/>"+item.nip_baru+" / "+item.tmt_pns+"</td>";
					...
					var sek = item.ibel;
					table = table+ "<td><div>";
					table = table+ "<div style='float:left; width:65px;'>No.SIB</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'><b>"+sek.nomor_surat+"</b></div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Tgl.SIB</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'>"+sek.tg_surat+"</div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Jenjang</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'>"+sek.nama_jenjang+"</div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Jurusan</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<span><div style='display:table;'>"+sek.nama_pendidikan+"</div></span>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Sekolah</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'>"+sek.nama_sekolah+"</div>";
					table = table+ "</div>";
					table = table+ "<div style='clear:both'>";
					table = table+ "<div style='float:left; width:65px;'>Lokasi</div>";
					table = table+ "<div style='float:left; width:10px;'>:</div>";
					table = table+ "<div style='float:left;'>"+sek.lokasi_sekolah+"</div>";
					table = table+ "</div>";
					table = table+ "</td><td>";
					...
					table = table+ '<a href="#" class="label label-default" onclick="zoom_dok(\'penyesuaian_ijazah\','+accIb.idd+',\''+item.nip_baru+'\');return false;"><i class="fa fa-search fa-fw"></i> Zoom</a>';
										}
					table = table+ "</p>";
					table = table+ "</div>";
					...
					table = table+ "</div>";
									...
									table = table+ "<div>";
									table = table+ "<div style='float:left; width:135px;'>Tanggal Sertifikat</div>";
									table = table+ "<div style='float:left; width:10px;'>:</div>";
									table = table+ "<div style='float:left;'>"+accIb.tanggal_surat+"</div>";
									table = table+ "</div>";
									table = table+ "<div style='clear:both'>";
									table = table+ "<div style='float:left; width:135px;'>Nomor Sertifikat</div>";
									table = table+ "<div style='float:left; width:10px;'>:</div>";
...
function ppost(idd,act){
	...
	$('#sb_act').attr('action','<?=site_url();?>appbangrir/pi_edok/alih');
	...
	var tab = tab + '<input type="hidden" name="asal" value="appbangrir/pi_daftar">';
