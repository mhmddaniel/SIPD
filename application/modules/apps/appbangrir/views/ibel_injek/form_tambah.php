...
<div class="row" id="pageForm">
	<div class="col-lg-12" id="colForm">
		<div class="panel panel-info">
			<div class="panel-heading">
				...
				<a class="btn btn-info btn-xs pull-right" href="<?=site_url();?>module/appbangrir/ibel_proses"><i class="fa fa-close fa-fw"></i></a>
			</div><!-- /.panel-heading -->
	...
	<span class="input-group-btn"><button class="btn btn-default" type="button" onclick="cari_nip();"><i class="fa fa-search"></i></button></span>
					...
					<div id="tbForm" style="text-align:right;"><div class="col-lg-12" style="padding-right:0px;">
						...
						<a class="btn btn-default" href="<?=site_url();?>module/appbangrir/ibel_proses"><i class="fa fa-fast-backward fa-fw"></i> Batal...</a>
					</div></div>
			</div><!-- /.panel-body -->
		</div><!-- /.panel-default -->
	</div><!-- /.col-lg-12 -->
...
function cari_nip(){
	...
	if(lnt==18){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appbangrir/ibel_injek/cari_nip",
				data:{"nip":nip},
				beforeSend:function(){	
					$('#fNIP').html('<p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p>');
				},
				success:function(data){
					$('#pageFormTo').attr('action','<?=site_url();?>appbangrir/ibel_injek/tambah_aksi');
...
function simpan(){
	jQuery.post($("#pageFormTo").attr('action'),$("#pageFormTo").serialize(),function(data){
		...
		if(arr_result[0]=='sukses'){
			location.href = '<?=site_url();?>module/appbangrir/ibel_proses';
		} else {
