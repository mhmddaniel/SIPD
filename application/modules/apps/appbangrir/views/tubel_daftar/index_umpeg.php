...
function setForm(tujuan,idd,no){
	var kop = []; 
	kop['form_tambah'] = "FORM TAMBAH PEGAWAI PEMOHON IJIN BELAJAR"; 
	kop['form_sekolah'] = "FORM IJIN BELAJAR"; 
	kop['form_hapus'] = "FORM HAPUS PEGAWAI PEMOHON"; 
	var act = []; 
	act['form_tambah'] = "<?=site_url();?>appbangrir/tubel_daftar/tambah_aksi"; 
	act['form_sekolah'] = "<?=site_url();?>appbangrir/tubel_daftar/sekolah_aksi"; 
	act['form_hapus'] = "<?=site_url();?>appbangrir/tubel_daftar/hapus_aksi"; 
	var btt = [];
	...
	btt['form_hapus'] = "<button id='btAct' type=sumbit class='btn btn-danger' onclick='ajukan(); return false;'><i class='fa fa-trash fa-fw'></i> Hapus</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appbangrir/tubel_daftar/"+tujuan,
			data:{"idd": idd,"no": no },
			beforeSend:function(){	
				$("#pageKonten").hide();
...
function gridpagingA(hal){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appbangrir/tubel_daftar/getdata",
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_sekolah\','+item.id_tubel+','+no+');return false;"><i class="fa fa-pencil fa-fw"></i> Edit data</a></li>';
						}
						if(item.hapus=="ya"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="setForm(\'form_hapus\','+item.id_tubel+','+no+');return false;"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
						}
						if(item.hapus=="ya" || item.status=="draft"){
						table = table+ '<li role="presentation" class="divider">';
						}
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_tubel+');return false;"><i class="fa fa-search fa-fw"></i> Lihat detail</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
//					table = table+ "<td style='padding:3px;'><b>"+item.nama_pegawai+"</b> ("+item.gender+")<br/>"+item.tempat_lahir+", "+item.tanggal_lahir+"<br/>"+item.nip_baru+" / "+item.tmt_pns+"</td>";
					...
					table = table+ '<a href="#" class="label label-default" onclick="zoom_dok(\'tubel\','+accIb.idd+',\''+item.nip_baru+'\');return false;"><i class="fa fa-search fa-fw"></i> Zoom</a>';
										}
					table = table+ "</p>";
					table = table+ "</div>";
...
function ppost(idd,act){
	...
	$('#sb_act').attr('action','<?=site_url();?>appbangrir/tubel_edok/alih');
	...
	var tab = tab + '<input type="hidden" name="asal" value="appbangrir/tubel_daftar">';
