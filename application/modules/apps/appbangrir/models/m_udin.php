<?php
class M_udin extends CI_Model{
	function __construct(){
		parent::__construct();
	}
//////////////////////////////////////////////////////////////////////////////////
	function hitung_udin($cari,$pns="all",$unor="all",$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur="all",$mkcpns="all",$stib="all"){
	...
	function get_udin($cari,$mulai,$batas,$pns="all",$unor="all",$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur="all",$mkcpns="all",$stib="all"){
			$ttIni = date('Y')."-".date('m')."-01";
			if($pns=="all"){	$iPns = "";	} elseif($pns=="cpns"){	$iPns = "AND e.tmt_pns='0000-00-00'";	} else {	$iPns = "AND e.tmt_pns!='0000-00-00'";	}
			if($kode==""){
				if($unor=="all"){	$iUnor = "";	} else {	/*$iUnor = "AND n.id_unor IN ($unor)";*/	$iUnor = "AND n.id_pegawai IN (SELECT id_pegawai FROM r_pegawai_aktual WHERE id_unor IN ($unor))";	}
	...
	function ini_udin($idd){
		$sqlstr="SELECT a.*, b.nama_pegawai, b.nip_baru,c.nomenklatur_pada FROM r_peg_ujian_dinas_aju a 
		LEFT JOIN r_pegawai b ON (a.id_pegawai=b.id_pegawai) 
		LEFT JOIN m_unor c ON (a.id_unor=c.id_unor) 
		WHERE a.id_udin='$idd'";
	...
	function ini_acc($idd){
		$sqlstr="SELECT a.* FROM r_peg_ujian_dinas a WHERE a.id_udin='$idd'";
		$acc = $this->db->query($sqlstr)->row();
		return $acc;
	}
//////////////////////////////////////////////////////////////////////////////////
	function get_catatan($idd,$stct=""){
		$this->db->from('r_peg_ujian_dinas_catatan');
	...
	function save_catatan($idd,$isi){
		...
		if($isi['id_catatan']==""){
		$this->db->set('id_udin',$idd);
		$this->db->insert('r_peg_ujian_dinas_catatan');
		} else {
		$this->db->where('id_catatan',$isi['id_catatan']);
		$this->db->update('r_peg_ujian_dinas_catatan');
		}
	}
	...
	function hapus_catatan($isi){
		$this->db->where('id_catatan',$isi['id_catatan']);
		$this->db->delete('r_peg_ujian_dinas_catatan');
	}
//////////////////////////////////////////////////////////////////////////////////
	...
	function ajukan_pemohon($idd){
		...
		$this->db->where('id_udin',$idd);
		$this->db->update('r_peg_ujian_dinas_aju');
	}
	...
	function acc_pemohon($isi){
		...
		$this->db->where('id_udin',$isi['id_udin']);
		$this->db->update('r_peg_ujian_dinas_aju');

		$peg = $this->ini_udin($isi['id_udin']);
		$tgl = date("Y-m-d", strtotime($isi['tanggal']));
		$this->db->set('id_udin',$isi['id_udin']);
		$this->db->set('id_pegawai',$peg->id_pegawai);
		$this->db->set('nomor_sertifikat',$isi['nomor']);
		$this->db->set('tanggal_sertifikat',$tgl);
	...
	function ini_sekolah($idd){
		$sqlstr="SELECT a.* FROM r_peg_ujian_dinas_sekolah a WHERE a.id_udin='$idd'";
		$peg = $this->db->query($sqlstr)->row();
		return $peg;
	}
	function sekolah_edit($isi){
		...
		$this->db->where('id_udin_sekolah',$isi['id_udin_sekolah']);
		$this->db->update('r_peg_ujian_dinas_sekolah');
	}
	function sekolah_tambah($isi){
		...
		$this->db->set('gelar_belakang',$isi['gelar_belakang']);
		$this->db->set('id_udin',$isi['id_udin']);
		$this->db->insert('r_peg_ujian_dinas_sekolah');
	}
//////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////
	function cek_dokumen($id_udin,$tipe){
		$this->db->from('r_peg_ujian_dinas_dokumen');
		$this->db->where('id_udin',$id_udin);
	...
	function simpan_dokumen($nip_baru,$nama_file,$tipe,$idd){
		...
		$hal = count($ini)+1;
			...
			$this->db->set('udin_file',$nama_file);
			$this->db->set('halaman',$hal);
//			$this->db->set('id_reff',$idd);
			$this->db->insert('r_peg_ujian_dinas_dokumen');
...
*/
	...
	function rename_dokumen($idd,$nama){
		$this->db->set('udin_file',$nama);
		$this->db->where('id_udin_dokumen',$idd);
		$this->db->update('r_peg_ujian_dinas_dokumen');
	}
	...
	function hapus_dokumen($idd,$id_peg,$komponen,$id_reff){
		$this->db->delete('r_peg_ujian_dinas_dokumen', array('id_udin_dokumen' => $idd));
		
		$dok = $this->cek_dokumen($id_reff,$komponen);
		foreach($dok AS $key=>$val){
			$sqlstr="UPDATE r_peg_ujian_dinas_dokumen SET halaman='".($key+1)."' WHERE id_udin_dokumen='".$val->id_udin_dokumen."'";		
			$this->db->query($sqlstr);
		}
		return $dok;
	}
	...
	function edit_keterangan_dokumen($isi){
		...
		$this->db->where('id_udin_dokumen',$isi['idd']);
		$this->db->update('r_peg_ujian_dinas_dokumen');
	}
