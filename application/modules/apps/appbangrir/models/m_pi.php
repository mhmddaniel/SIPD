<?php
class M_pi extends CI_Model{
	function __construct(){
		parent::__construct();
	}
//////////////////////////////////////////////////////////////////////////////////
	function hitung_pi($cari,$pns="all",$unor="all",$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur="all",$mkcpns="all",$stib="all"){
		...
		FROM r_peg_penyesuaian_ijazah_aju n
	...
	function get_pi($cari,$mulai,$batas,$pns="all",$unor="all",$kode,$pkt,$jbt,$ese,$tugas,$gender,$agama,$status,$jenjang,$umur="all",$mkcpns="all",$stib="all"){
			$ttIni = date('Y')."-".date('m')."-01";
			if($pns=="all"){	$iPns = "";	} elseif($pns=="cpns"){	$iPns = "AND e.tmt_pns='0000-00-00'";	} else {	$iPns = "AND e.tmt_pns!='0000-00-00'";	}
			if($kode==""){
				if($unor=="all"){	$iUnor = "";	} else {	/*$iUnor = "AND n.id_unor IN ($unor)";*/	$iUnor = "AND n.id_pegawai IN (SELECT id_pegawai FROM r_pegawai_aktual WHERE id_unor IN ($unor))";	}
	...
	function ini_pi($idd){
		$sqlstr="SELECT a.*, b.nama_pegawai, b.nip_baru, c.nomenklatur_pada FROM r_peg_penyesuaian_ijazah_aju a 
		LEFT JOIN r_pegawai b ON (a.id_pegawai=b.id_pegawai) 
		LEFT JOIN m_unor c ON (a.id_unor=c.id_unor) 
		WHERE a.id_pi='$idd'";
	...
	function ini_acc($idd){
		$sqlstr="SELECT a.* FROM r_peg_penyesuaian_ijazah a WHERE a.id_pi='$idd'";
		$acc = $this->db->query($sqlstr)->row();
		return $acc;
	}
//////////////////////////////////////////////////////////////////////////////////
	function get_catatan($idd,$stct=""){
		$this->db->from('r_peg_penyesuaian_ijazah_catatan');
	...
	function save_catatan($idd,$isi){
		...
		if($isi['id_catatan']==""){
		$this->db->set('id_pi',$idd);
		$this->db->insert('r_peg_penyesuaian_ijazah_catatan');
		} else {
		$this->db->where('id_catatan',$isi['id_catatan']);
		$this->db->update('r_peg_penyesuaian_ijazah_catatan');
		}
	}
	...
	function hapus_catatan($isi){
		$this->db->where('id_catatan',$isi['id_catatan']);
		$this->db->delete('r_peg_penyesuaian_ijazah_catatan');
	}
//////////////////////////////////////////////////////////////////////////////////
	function tambah_pemohon($pegawai,$idib,$nmr,$tgl){
		$tgl = date("Y-m-d", strtotime($tgl));
		$this->db->set('id_pegawai',$pegawai->id_pegawai);
		$this->db->set('id_ibel',$idib);
		$this->db->set('nomor_ijazah',$nmr);
		$this->db->set('tanggal_ijazah',$tgl);
	...
	function edit_pemohon($isi){
		$tgl = date("Y-m-d", strtotime($isi['tanggal_ijazah']));
		$this->db->set('id_ibel',$isi['id_ibel']);
		$this->db->set('nomor_ijazah',$isi['nomor_ijazah']);
		$this->db->set('tanggal_ijazah',$tgl);
		$this->db->where('id_pi',$isi['id_pi']);
		$this->db->update('r_peg_penyesuaian_ijazah_aju');
	}
...
//////////////////////////////////////////////////////////////////////////////////
	function cek_dokumen($id_pi,$tipe){
	...
	function simpan_dokumen($nip_baru,$nama_file,$tipe,$idd){
		...
		$hal = count($ini)+1;
			...
			$this->db->set('pi_file',$nama_file);
			$this->db->set('halaman',$hal);
//			$this->db->set('id_reff',$idd);
			$this->db->insert('r_peg_penyesuaian_ijazah_dokumen');
...
*/
	...
	function rename_dokumen($idd,$nama){
		$this->db->set('pi_file',$nama);
		$this->db->where('id_pi_dokumen',$idd);
		$this->db->update('r_peg_penyesuaian_ijazah_dokumen');
	}
	...
	function hapus_dokumen($idd,$id_peg,$komponen,$id_reff){
		$this->db->delete('r_peg_penyesuaian_ijazah_dokumen', array('id_pi_dokumen' => $idd));
		
		$dok = $this->cek_dokumen($id_reff,$komponen);
		foreach($dok AS $key=>$val){
			$sqlstr="UPDATE r_peg_penyesuaian_ijazah_dokumen SET halaman='".($key+1)."' WHERE id_pi_dokumen='".$val->id_pi_dokumen."'";		
			$this->db->query($sqlstr);
		}
		return $dok;
	}
	...
	function edit_keterangan_dokumen($isi){
		...
		$this->db->where('id_pi_dokumen',$isi['idd']);
		$this->db->update('r_peg_penyesuaian_ijazah_dokumen');
	}
