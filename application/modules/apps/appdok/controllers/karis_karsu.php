...
///////////////////////////////////////////////////////////////////////////////////
	function edit(){
		...
		$data['isi'] = $this->m_profil->ini_karis_karsu($_POST['idd']);
		if(empty($data['isi'])) {
			$data['aksi'] = "input";
			$data['token'] = sha1('data_input_kariskarsu_'.$data['id_pegawai']);
		}else {
			$data['aksi'] = "edit";
			$data['token'] = sha1('data_edit_kariskarsu_'.$data['id_pegawai']);
		}
	...
	function hapus(){
		...
		$data['isi'] = $this->m_profil->ini_karis_karsu($_POST['idd']);
		$data['hapus'] = "ya";
		$data['token'] = sha1('data_hapus_kariskarsu_'.$data['id_pegawai']);
		$this->load->view('karis_karsu/form',$data);
	}
