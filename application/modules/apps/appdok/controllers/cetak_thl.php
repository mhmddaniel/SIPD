...
class Cetak_thl extends MX_Controller {
...
//		$this->load->model('appbkpp/m_pegawai');
	...
	function createcv($id_pegawai=false){
		...
		$this->mypdf->Image($fotopath, 160, 33, '', 30, '', '', '', true, 100);

		$logo = base_url()."assets/images/logo-kota-tangerang.png";
	...
	function gethtml($id_pegawai=false){
		...
		return $this->load->view('cv/template_thl',array('data'=>$html),true );
	}
	function getdata($id_pegawai=false){
		...
		$html['head'] = $this->load->view('cv/head_thl',array('data'=>$data['head']),true );
 		$data['pribadi'] = Modules::run("appbkpp/profile/ini_pegawai",$id_pegawai);// 	Modules::run("datamodel/pegawai/get_peg_biodata",$id_pegawai);
		$html['pribadi'] = $this->load->view('cv/pribadi_thl',array('data'=>$data['pribadi']),true );
