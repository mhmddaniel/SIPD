...
var mkcpns = $('#a_mkcpns').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appevjab/pegawai/getaktif",
		data:{"bulan": <?=date('m');?>, "tahun": <?=date('Y');?>,"hal": hal, "batas": batas,"cari":cari,"pns":pns,"kode":kode,"pkt":pkt,"jbt":jbt,"ese":ese,"tugas":tugas,"gender":gender,"agama":agama,"status":status,"jenjang":jenjang,"umur":umur,"mkcpns":mkcpns},
		beforeSend:function(){	
	...
	//tombol aksi-->
					table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
						...
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_pegawai+',\'module/appevjab/profile_pegjab/alih\');return false;"><i class="fa fa-binoculars fa-fw"></i> Rincian Jabatan Pegawai</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
...
function ppost(idd,act){
	...
	var tab = tab + '<input type="hidden" name="asal" value="appevjab/pegawai/aktif_umpeg">';
