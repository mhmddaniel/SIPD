	...
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">
					<div class="row">
						<div class="col-lg-10">
							<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" data-toggle="dropdown"><i class="fa fa-briefcase fa-fw"></i></button>
...
<table class="table table-striped table-bordered table-hover">
	<thead>
		<tr>
			...
			<th style="width:250px;text-align:center; vertical-align:middle">FISIK / MENTAL</th>
			<th style="text-align:center; vertical-align:middle">PENYEBAB</th>
		</tr>
	</thead>
	<tbody>
	<?php
	foreach($resiko AS $key=>$val){
	?>
		<tr>
			<td><?=$key+1;?></td>
			<td>
...
<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setF('formedit',<?=$val->id_resiko;?>,<?=$key+1;?>);"><i class="fa fa-edit fa-fw"></i> Edit data</a></li>
<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setF('formhapus',<?=$val->id_resiko;?>,<?=$key+1;?>);"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>
</ul>
</div>
			</td>
			<td><?=$val->fisik;?></td>
...
function setF(tujuan,idd,no){
	var kop = []; 
	kop['formtambah'] = "FORM TAMBAH RESIKO BAHAYA"; 
	kop['formedit'] = "FORM EDIT RESIKO BAHAYA"; 
	kop['formhapus'] = "FORM HAPUS RESIKO BAHAYA"; 
	var act = []; 
	act['formtambah'] = "<?=site_url();?>appevjab/jabfung/resiko_tambah_aksi";
	act['formedit'] = "<?=site_url();?>appevjab/jabfung/resiko_edit_aksi";
	act['formhapus'] = "<?=site_url();?>appevjab/jabfung/resiko_hapus_aksi";
	var btt = []; 
	...
	btt['formhapus'] = "<button id='btAct' type=sumbit class='btn btn-danger' onclick='ajukan(); return false;'><i class='fa fa-trash fa-fw'></i> Hapus</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appevjab/jabfung/resiko_"+tujuan,
			data:{"idd": idd,"no": no },
			beforeSend:function(){	
				$("#pageKonten").hide();
