...
var batas = $('#item_length_rincian').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appevjab/satu_js/getrincian_isi",
							...
							$.each( item.pejabat, function(index, item2){
								pemangku = pemangku+'<div style="clear:both;">';
									pemangku = pemangku+'<div>';
										pemangku = pemangku+'<div style="display:table;"><span class="btn btn-default btn-xs" onclick="detil2('+item2.id_pegawai+',\'appbkpp/profile/pns_ini\',\'tidak\');"><i class="fa fa-binoculars fa-fw"></i></span> <b>'+item2.nama_pegawai+'</b></div>';
									pemangku = pemangku+'</div>';
									pemangku = pemangku+'<div style="clear:both;">';
...
//					table = table+ "</td>";
	//tombol aksi<--
					...
					table = table+ "<td style='padding:3px;'><span class='btn btn-default btn-xs' onclick=\"detil2("+item.id_pegawai+",\'appbkpp/profile/pns_ini\',\'tidak\');\"><i class='fa fa-binoculars fa-fw'></i></span> <b>"+item.nama_pegawai+"</b><br>"+item.nip_baru+"<br>"+item.nama_pangkat+" / "+item.nama_golongan+"</td>";
