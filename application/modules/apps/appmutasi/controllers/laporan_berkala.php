...
class Laporan_berkala extends MX_Controller {
  ...
  public function index()  {
		...
		$query = $this->m_dashboard->get_panel(1,$data['bulan'],$data['tahun']);
		$data['jabatan'] =json_decode(@$query->meta_value);

		$query = $this->m_dashboard->get_panel(2,$data['bulan'],$data['tahun']);
		$data['pendidikan'] =json_decode(@$query->meta_value);

		$query = $this->m_dashboard->get_panel(3,$data['bulan'],$data['tahun']);
		$data['golongan'] =json_decode(@$query->meta_value);

		$query = $this->m_dashboard->get_panel(4,$data['bulan'],$data['tahun']);
		$data['umur'] =json_decode(@$query->meta_value);

		$query = $this->m_dashboard->get_panel(5,$data['bulan'],$data['tahun']);
		$data['mkcpns'] =json_decode(@$query->meta_value);

		$query = $this->m_dashboard->get_panel(6,$data['bulan'],$data['tahun']);
		$data['unor'] =json_decode(@$query->meta_value);

		$query = $this->m_dashboard->get_panel(7,$data['bulan'],$data['tahun']);
		$data['bup'] =json_decode(@$query->meta_value);

		$sess = $this->session->userdata('logged_in');
		$group_name = $sess['group_name'];
		if($group_name=="mutasi"){
			$rd = "index";
...
/////////////////////  ---PREPARE---   /////////////////////////////////
		...
		$this->m_dashboard->isi_pns($dBulan,$tahun);							// ISI TABEL PNS & CPNS DULU
////////////////////////////////////////////////////////////////////////
		$jabatan = $this->dropdowns->jenis_jabatan();
		$data['jabatan'] = array();
			foreach($jabatan as $key=>$val){	if($key!=""){
					$jl = $this->m_dashboard->hitung_pegawai_jabatan($key,"l","all",$dBulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_jabatan($key,"p","all",$dBulan,$tahun);
		...
		$df = $this->m_dashboard->satu(1,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
		$pendidikan = $this->dropdowns->kode_jenjang_pendidikan();
		$data['pendidikan'] = array();
			foreach($pendidikan as $key=>$val){	if($key!=""){
					$jl = $this->m_dashboard->hitung_pegawai_pendidikan($val,"l","all",$dBulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_pendidikan($val,"p","all",$dBulan,$tahun);
		...
		$df = $this->m_dashboard->satu(2,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
		$golongan = $this->dropdowns->kode_golongan_pangkat();
		$data['golongan'] = array();
			foreach($golongan as $key=>$val){	if($key!=""){
					$jl = $this->m_dashboard->hitung_pegawai_golongan($key,"l","all",$dBulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_golongan($key,"p","all",$dBulan,$tahun);
		...
		$df = $this->m_dashboard->satu(3,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
		...
		$data['umur'] = array();
			foreach($umur as $key=>$val){	if($key!=""){
					...
					$batas = $umur_db[$key];
					$jl = $this->m_dashboard->hitung_pegawai_umur($batas,"l","all",$dBulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_umur($batas,"p","all",$dBulan,$tahun);
					@$data['umur'][$key]->l = $jl;
					@$data['umur'][$key]->p = $jp;
		...
		$df = $this->m_dashboard->satu(4,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
		...
		$data['mkcpns'] = array();
			foreach($mkcpns as $key=>$val){	if($key!=""){
					...
					$batas = $mkcpns_db[$key];
					$jl = $this->m_dashboard->hitung_pegawai_mkcpns($batas,"l","all",$dBulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_mkcpns($batas,"p","all",$dBulan,$tahun);
		...
		$df = $this->m_dashboard->satu(5,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
			$dueDate = $tahun."-".$dBulan."-01";
		$data['unor'] = $this->m_unor->gettree(0,5,$dueDate);
			foreach($data['unor'] AS $key=>$val){
				$j_all = $this->m_dashboard->hitung_pegawai($val->kode_unor,"all","all","all",$dBulan,$tahun);
				$j_pns = $this->m_dashboard->hitung_pegawai($val->kode_unor,"pns","all","all",$dBulan,$tahun);
				$j_cpns = $this->m_dashboard->hitung_pegawai($val->kode_unor,"cpns","all","all",$dBulan,$tahun);
...
//				$data['where'] = ($key==0)?$data['where']."kode_unor NOT LIKE '".$val->kode_unor."%'":$data['where']." AND kode_unor NOT LIKE '".$val->kode_unor."%'";
		...
		$df = $this->m_dashboard->satu(6,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
		$data['bup'] = array();
		for($i=0;$i<1;$i++){
			$th = date('Y')+$i;
			@$data['bup'][$i]->tahun = $th;
			@$data['bup'][$i]->guru_l = $this->prediksi_pensiun($th,'jft-guru','l',$dBulan,$tahun);
			@$data['bup'][$i]->guru_p = $this->hitung_cuti(1);
			@$data['bup'][$i]->guru_j = $this->prediksi_pensiun($th,'jft-guru','p',$dBulan,$tahun);
		...
		$df = $this->m_dashboard->satu(7,$jj,$vll,$dBulan,$tahun);
////////////////////////////////////////////////////////////////////////
//		redirect(site_url('module/appbkpp/dashboard'));
	...
	function orphan(){
		...
		$trn = array();
		foreach($data['unor'] AS $key=>$val){
			$kl = $this->m_dashboard->cek_anak($val->kode_unor,$dueDate);
			$rn=array();
		...
		$trn = implode(",",$trn);
		$data['ky'] = $this->m_dashboard->cek_orphan($trn);
  ...
  public function unor() {
	...
	$thh = $_POST['tahun'];
		...
		$data['golongan'] = array();
			foreach($golongan as $key=>$val){	if($key!=""){
					$pns_l = $this->m_dashboard->hitung_pegawai($unor->kode_unor,'pns',$key,'l',$bll,$thh);
					$pns_p = $this->m_dashboard->hitung_pegawai($unor->kode_unor,'pns',$key,'p',$bll,$thh);
					$cpns_l = $this->m_dashboard->hitung_pegawai($unor->kode_unor,'cpns',$key,'l',$bll,$thh);
					$cpns_p = $this->m_dashboard->hitung_pegawai($unor->kode_unor,'cpns',$key,'p',$bll,$thh);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  public function umpeg() {
		...
		$data['golongan'] = array();
			foreach($golongan as $key=>$val){	if($key!=""){
					$jl = $this->m_dashboard->hitung_pegawai_golongan($key,"l",$acl,$bulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_golongan($key,"p",$acl,$bulan,$tahun);
					@$data['golongan'][$key]->nama = $val;
					@$data['golongan'][$key]->l = $jl;
					@$data['golongan'][$key]->p = $jp;
			}}
		...
		$data['pendidikan'] = array();
			foreach($pendidikan as $key=>$val){	if($key!=""){
					$jl = $this->m_dashboard->hitung_pegawai_pendidikan($val,"l",$acl,$bulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_pendidikan($val,"p",$acl,$bulan,$tahun);
					@$data['pendidikan'][$key]->nama = $val;
					@$data['pendidikan'][$key]->l = $jl;
					@$data['pendidikan'][$key]->p = $jp;
			}}
		...
		$data['jabatan'] = array();
			foreach($jabatan as $key=>$val){	if($key!=""){
					$jl = $this->m_dashboard->hitung_pegawai_jabatan($key,"l",$acl,$bulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_jabatan($key,"p",$acl,$bulan,$tahun);
					@$data['jabatan'][$key]->nama = $val;
					@$data['jabatan'][$key]->l = $jl;
					@$data['jabatan'][$key]->p = $jp;
			}}
		...
		$data['umur'] = array();
			foreach($umur as $key=>$val){	if($key!=""){
					@$data['umur'][$key]->nama = $val;
					$batas = $umur_db[$key];
					$jl = $this->m_dashboard->hitung_pegawai_umur($batas,"l",$acl,$bulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_umur($batas,"p",$acl,$bulan,$tahun);
					@$data['umur'][$key]->l = $jl;
					@$data['umur'][$key]->p = $jp;
			}}
		...
		$data['mkcpns'] = array();
			foreach($mkcpns as $key=>$val){	if($key!=""){
					@$data['mkcpns'][$key]->nama = $val;
					$batas = $mkcpns_db[$key];
					$jl = $this->m_dashboard->hitung_pegawai_mkcpns($batas,"l",$acl,$bulan,$tahun);
					$jp = $this->m_dashboard->hitung_pegawai_mkcpns($batas,"p",$acl,$bulan,$tahun);
					@$data['mkcpns'][$key]->l = $jl;
					@$data['mkcpns'][$key]->p = $jp;
			}}
  	...
  	if(isset($_POST['refresh'])){
						...
						foreach($data['unor'] AS $key=>$val){
							...
							foreach($kl AS $key2=>$val2){
								$kl2 = $this->m_dashboard->cek_anak($val2->kode_unor.".",$dueDate);
								if(empty($kl2)){
