...
class Kpo_edok extends MX_Controller {
...
///////////////////////////////////////////////////////////////////////////////////
	function alih(){
		...
		$kpo = $this->m_kpo->ini_kpo($_POST['idd']);
		if($kpo->status=="draft" && $kpo->draft==null){	$this->m_kpo->draft_pemohon($_POST['idd']);	}

		redirect("module/appmutasi/kpo_edok");
	}
	function index(){
		...
		$this->load->view('kpo_edok/index',$data);
	}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	function ijin(){
		...
		$data['komponen'] = "ijin";
		$data['kpo'] = $this->m_kpo->ini_kpo($data['idd']);
		$data['surat'] = json_decode($data['kpo']->ijin_pimpinan);

		$data['isi'] = $this->m_kpo->cek_dokumen($data['idd'],$data['komponen']);
		if(!empty($data['isi'])){
			$ffl = "assets/media/kpo/".$peg->nip_baru."/".$data['komponen']."/".$data['isi'][0]->kpo_file;
			$data['raw'] = pathinfo($ffl, PATHINFO_EXTENSION);
			if($data['raw']=="jpg"){
				$data['thumb'] = "assets/media/kpo/".$peg->nip_baru."/".$data['komponen']."/thumb_".$data['isi'][0]->kpo_file;
			} else {
		...
		$this->load->view('kpo_edok/ijin',$data);
	}
	function ijin_formedit(){
		$data['idd'] = $this->session->userdata('idd');//id_ibel
		$surat = $this->m_kpo->ini_kpo($data['idd']);
		$data['isi'] = json_decode($surat->ijin_pimpinan);
		$this->load->view('kpo_edok/ijin_formedit',$data);
	}
	function ijin_edit_aksi(){
		$this->m_kpo->ijin_edit($_POST);
		echo "sukses";
	}
	function ijin_upload(){
		...
		$surat = $this->m_kpo->ini_kpo($data['idd']);
		$data['isi'] = json_decode($surat->ijin_pimpinan);
		$data['row'] = $this->m_kpo->cek_dokumen($data['idd'],$_POST['komponen']);
		$data['nip_baru'] = $_POST['id_pegawai'];
		
		foreach($data['row'] AS $key=>$val){
			$ffl = "assets/media/kpo/".$data['nip_baru']."/".$data['komponen']."/".$val->kpo_file;
			$raw = pathinfo($ffl, PATHINFO_EXTENSION);
			if($raw=="jpg"){
				@$data['row'][$key]->thumb = "assets/media/kpo/".$data['nip_baru']."/".$data['komponen']."/thumb_".$val->kpo_file;
			} else {
				@$data['row'][$key]->thumb = "assets/media/foto/default/doc.png";
			}
		}

		$this->load->view('kpo_edok/ijin_upload',$data);
	}
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	function sk_cpns(){
	...
	function saveupload(){
		if(strlen($_FILES['artikel_file']['name'])>0){
				...
				if($ext=="jpg"){
						...
						//$config['thumb_marker']='';
						$config['source_image'] = 'assets/media/kpo/'.$id_pegawai.'/'.$komponen.'/'.$result['raw'].".".$ext;
						$config['new_image'] = 'assets/media/kpo/'.$id_pegawai.'/'.$komponen.'/thumb_'.$result['raw'].".".$ext;
...
////////////////////////////////
				...
				$nmF = $idd."_".$nmB.".".$ext;
				rename("assets/media/kpo/".$id_pegawai."/".$komponen."/".$result['raw'].".".$ext,"assets/media/kpo/".$id_pegawai."/".$komponen."/".$nmF);
				if($ext=="jpg"){
						rename("assets/media/kpo/".$id_pegawai."/".$komponen."/thumb_".$result['raw'].".".$ext,"assets/media/kpo/".$id_pegawai."/".$komponen."/thumb_".$nmF);
				}
				$this->m_kpo->rename_dokumen($result['idd'],$nmF);
////////////////////////////////
	...
	function uploadFile($id_pegawai,$nama_file,$komponen,$idd){
		$this->load->helper('file');
			$path="assets/media/kpo/".$id_pegawai."/";
			if(!file_exists($path)){	mkdir($path,755);	}
			$path2="assets/media/kpo/".$id_pegawai."/".$komponen."/";
...
//		$config['max_size']	= '512';
		...
		if ( ! $this->upload->do_upload('artikel_file'))		{
			...
			$ttpp = $dn['raw_name'];
			$idB = $this->m_kpo->simpan_dokumen($id_pegawai,$nama_file,$komponen,$idd);
			$data['raw'] = $ttpp;
			$data['status'] = "success";
			$data['idd'] = $idB;
//			$gg = Modules::run("appdok/pantau/kontrol",$id_pegawai);
	...
	function hapus_dok(){
		...
		$dok = $this->m_kpo->ini_dokumen($id_dokumen);
		unlink("assets/media/kpo/".$id_pegawai."/".$komponen."/".$dok->kpo_file);
		unlink("assets/media/kpo/".$id_pegawai."/".$komponen."/thumb_".$dok->kpo_file);
		$hp = $this->m_kpo->hapus_dokumen($id_dokumen,$id_pegawai,$komponen,$_POST['idd']);

//		$gg = Modules::run("appdok/pantau/kontrol",$id_pegawai);
			echo "<b>Sukses...</b>";
	}
	function edit_keterangan_aksi(){
		$this->m_kpo->edit_keterangan_dokumen($_POST);
	...
	function zoom(){
		...
		$data['dokumen'] = $this->m_kpo->cek_dokumen($data['idd'],$_POST['komponen']);
		foreach($data['dokumen'] AS $key=>$val){
			$ffl = "assets/media/kpo/".$data['nip_baru']."/".$data['komponen']."/".$val->kpo_file;
			@$data['dokumen'][$key]->row = pathinfo($ffl, PATHINFO_EXTENSION);
		}
		$this->load->view('kpo_edok/zoom',$data);
	}
