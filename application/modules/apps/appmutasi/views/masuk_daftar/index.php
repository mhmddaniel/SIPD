...
function setForm(tujuan,idd,no){
	var kop = []; 
	kop['form_tambah'] = "FORM TAMBAH PEGAWAI PEMOHON MASUK"; 
	kop['form_edit'] = "FORM PENSIUN"; 
	kop['form_hapus'] = "FORM HAPUS PEGAWAI PEMOHON"; 
	var act = []; 
	act['form_tambah'] = "<?=site_url();?>appmutasi/masuk_daftar/tambah_aksi"; 
	act['form_edit'] = "<?=site_url();?>appmutasi/masuk_daftar/edit_aksi"; 
	act['form_hapus'] = "<?=site_url();?>appmutasi/masuk_daftar/hapus_aksi"; 
	var btt = [];
	...
	btt['form_hapus'] = "<button id='btAct' type=sumbit class='btn btn-danger' onclick='ajukan(); return false;'><i class='fa fa-trash fa-fw'></i> Hapus</button>"; 
			$.ajax({
			type:"POST",
			url:"<?=site_url();?>appmutasi/masuk_daftar/"+tujuan,
			data:{"idd": idd,"no": no },
			beforeSend:function(){	
				$("#pageKonten").hide();
...
function gridpagingA(hal){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appmutasi/masuk_daftar/getdata",
...
//						if(item.status=="draft"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onClick="setForm(\'form_edit\','+item.id_masuk+','+no+');return false;"><i class="fa fa-pencil fa-fw"></i> Edit data</a></li>';
//						}
//						if(item.hapus=="ya"){
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="setForm(\'form_hapus\','+item.id_masuk+','+no+');return false;"><i class="fa fa-trash fa-fw"></i> Hapus data</a></li>';
//						}
						if(item.hapus=="ya" || item.status=="draft"){
						table = table+ '<li role="presentation" class="divider">';
						}
						table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="ppost('+item.id_masuk+',\'masuk_daftar/ini\');return false;"><i class="fa fa-search fa-fw"></i> Lihat detail</a></li>';
						table = table+ "</ul>";
						table = table+ "</div>";
					table = table+ "</td>";
	//tombol aksi<--
