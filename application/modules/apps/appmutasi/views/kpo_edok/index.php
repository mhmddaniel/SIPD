		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
								<div>
										<div style="float:left; width:90px;">Pangkat/Gol.</div>
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$val->pangkat_aju;?></div>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Jenis</div>
										<div style="float:left; width:5px;">:</div>
										<span><div style="display:table;"><?=$jenis[$val->kode_jenis_kpo];?></div></span>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Tahun</div>
										<div style="float:left; width:5px;">:</div>
										<div style="float:left;"><?=$val->tahun_periode;?></div>
								</div>
								<div style="clear:both">
										<div style="float:left; width:90px;">Bulan</div>
										<div style="float:left; width:5px;">:</div>
										<span><div style="display:table;"><?=$bulan[$val->bulan_periode];?></div></span>
								</div>
			</div>
		</div><!-- /.panel -->
	</div><!-- /.col-lg-6 -->
...
function uppl(bttn,stts,dokumen){	
		...
		new AjaxUpload(btnUpload, {
			action: '<?=site_url();?>appmutasi/kpo_edok/saveupload',
			name: 'artikel_file',
...
function ok_hapus(tipe,idd){
	...
	var id_dokumen = $('#head_no_'+tipe).attr("data-id_dokumen");
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appmutasi/kpo_edok/hapus_dok",
...
function isi(dok,prs){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appmutasi/kpo_"+prs+"/"+dok,
...
function viewForm(komponen){
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appmutasi/kpo_edok/"+komponen+"_formedit",
				data:{"id_pegawai":"<?=$id_pegawai;?>"},
				beforeSend:function(){	
					$('#accordion').hide();
					$('#formedok').show();
...
function viewUppl(komponen,idd){
	...
	var nip_baru = $('#nip_baru').html();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appmutasi/kpo_edok/"+komponen+"_upload",
...
function satuket(idd){
	...
	var sub_keterangan = $('#sub_ket_dok_'+idd).val();
			$.ajax({
				type:"POST",
				url:"<?=site_url();?>appmutasi/kpo_edok/edit_keterangan_aksi",
...
function zoom_dox(komponen,idd){
	var nip_baru = $('#nip_baru').html();
	$('#sb_act').attr('action','<?=site_url();?>appmutasi/kpo_edok/zoom').attr('target','_blank');
	var tab = '<input type="hidden" name="komponen" value="'+komponen+'">';
	var tab = tab + '<input type="hidden" name="nip_baru" value="'+nip_baru+'">';	
	$('#sb_act').html(tab).submit();
}
