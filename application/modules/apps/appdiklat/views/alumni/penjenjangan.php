			...
			<div class="panel-heading">
						...
						<span class="dropdown"><button class="btn btn-default dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-graduation-cap fa-fw"></span></button>
							<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
								...
								<li role="presentation" class="active"><a href="#" role="menuitem"><i class="fa fa-graduation-cap fa-fw"></i> Diklat Penjenjangan</a></li>
								<li role="presentation"><a href="<?=site_url('module/appdiklat/alumni/kursus');?>" role="menuitem"><i class="fa fa-tasks fa-fw"></i> Kursus/Sertfiikasi dll</a></li>
							</ul>
							<span id="judul"> Daftar Alumni Diklat Penjenjangan</span>
						</span>
								...
								<div class="panel panel-default">
									<div class="panel-body">
										<div class="form-group">
											<label>Rumpun Diklat Struktural:</label>
												<select id="a_rumpun_diklat_struk" name="a_rumpun_diklat_struk" class="form-control" onchange="gridpaging(1);">
													<?php
														foreach($rumpun as $key=>$val){
															$selC = ($key==1)?" selected":"";
															if($key!=5 && $key!=""){	echo '<option value="'.$key.'" '.$selC.'>'.$val.'</option>';	}
														}
													?>
												</select>
										</div><!--/.form-group-->
...
var rumpun = $('#a_rumpun_diklat_struk').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>appdiklat/alumni/getdata",
	...
	//tombol aksi<--
					...
					table = table+ "<td>"+item.penyelenggara+"<br>"+item.tempat_diklat+"</td>";
...
function zoom_dok(nip_baru,idd){
	...
	var tab = '<input type="hidden" name="komponen" value="sertifikat_diklat">';
