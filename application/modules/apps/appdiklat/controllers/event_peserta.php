...
class Event_peserta extends MX_Controller {

	function __construct(){
		parent::__construct();
		$this->auth->restrict();
		$this->load->model('appdiklat/m_event_peserta');
...
///////////////////////////////////////////////////////////////////////////////////
	function index(){
		...
		$this->load->view('event_peserta/index',$data);
	}
	...
	function getdata(){
		...
		$rumpun = $_POST['rumpun'];
		$cari = $_POST['cari'];
		$data['count'] = $this->m_event_peserta->hitung_event($cari,$rumpun,$tahun);
		...
		if($data['count']==0){
			...
			$mulai=($hal-1)*$batas;
			$data['mulai']=$mulai+1;
			$data['hslquery'] = $this->m_event_peserta->get_event($cari,$rumpun,$tahun,$mulai,$batas);
	...
	function rinci(){
		...
		$data['id_rumpun'] = $idd[2];
		$data['tahun'] = $_POST['tahun'];
		$data['diklat'] = $this->m_event_peserta->ini_event($data['idd']);
		$this->load->view('event_peserta/rinci',$data);
	}
	function getpeserta(){
		...
		if($group_name=="pengelola"){
			...
			$user = $this->m_umpeg->ini_user($user_id);
				$dd=array("{","}");
			$unor=  "Bidang Perencanan Dan Pengembangan Iklim Penanaman Modal";
		} else {
			$unor="all";
		}

		$cari = $_POST['cari'];
		$data['count'] = $this->m_event_peserta->hitung_peserta($_POST['id_diklat_event'],$cari);
		...
		if($data['count']==0){
			...
			$mulai=($hal-1)*$batas;
			$data['mulai']=$mulai+1;
			$data['hslquery'] = $this->m_event_peserta->get_peserta($_POST['id_diklat_event'],$cari,$mulai,$batas);
