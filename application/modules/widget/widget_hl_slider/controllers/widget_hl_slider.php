	...
	public function index($id_widget,$id_wrapper,$opsi){
		...
		foreach ($data['daftar'] as $key=>$val) {
			...
			if(empty($ssdd)){
				...
				@$data['daftar'][$key]->imgslider="assets/media/slider/".$val->id_konten."/".@$ssdd[0]->foto;
				@$data['daftar'][$key]->imgthumbs="assets/media/slider/".$val->id_konten."/thumbs_".@$ssdd[0]->foto;
			}
		}
