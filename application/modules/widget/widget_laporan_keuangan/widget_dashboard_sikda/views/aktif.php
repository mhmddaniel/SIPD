...
<th style="width:250px;text-align:center; vertical-align:middle">NAMA PEGAWAI</th>
<th style="width:160px;text-align:center; vertical-align:middle">JABATAN</th>
...
var mkcpns = $('#a_mkcpns').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>widget_dashboard_sikda/getaktif",
		...
		success:function(data){
			if((data.hslquery.length)>0){
				...
				$.each( data.hslquery, function(index, item){
					...
					table = table+ "<td style='padding:3px;'><b>"+item.nama_pegawai+"</td>";
					if(item.tugas_tambahan=='xx' || item.tugas_tambahan=='') {
						table = table+ "<td style='padding:3px;'>" +item.nomenklatur_jabatan+"</td>";
					} else {
						table = table+ "<td style='padding:3px;'>" +item.nomenklatur_jabatan+" (<b>"+item.tugas_tambahan+"</b>)</td>";
					}
