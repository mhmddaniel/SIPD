...
class Web_pencarian extends MX_Controller {
	...
	public function gethasil()	{
		...
		if($er==FALSE || $es==FALSE){	
			$data['hslquery']= "xx_xx";	
				...
				$sess = $this->session->userdata('visit');
				$kata_kunci = @$sess['kata_kunci'];

				$dt = $this->m_pencarian->hitungpencarian($kata_kunci);
				...
				if($dt['count']==0){
					...
					$mulai=($hal-1)*$batas;
					$data['mulai']=$mulai+1;
					$data['hslquery'] = $this->m_pencarian->getpencarian($kata_kunci,$mulai,$batas);
					$d = array ('-','/','\\',',','.','#',':',';','\'','"','[',']','{','}',')','(','|','`','~','!','@','%','$','^','&','*','=','?','+',' ');
