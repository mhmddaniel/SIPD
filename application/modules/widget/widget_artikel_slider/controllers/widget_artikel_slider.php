...
class Widget_artikel_slider extends MX_Controller {
	...
	public function index($id_widget,$id_wrapper,$opsi)	{
		$data['idd']=$id_wrapper;
		$data['daftar'] = $this->m_artikel_slider->getwidget($id_widget,$id_wrapper);
	    $d = array ('-','/','\\',',','.','#',':',';','\'','"','[',']','{','}',')','(','|','`','~','!','@','%','$','^','&','*','=','?','+',' ');
		foreach ($data['daftar'] as $key=>$val) {
			...
			@$data['daftar'][$key]->imgslider=(empty($ssdd))?"assets/media/slider/default/slide_".($key+1).".jpg":"assets/media/slider/".$val->id_konten."/".@$ssdd[0]->foto;
		}
