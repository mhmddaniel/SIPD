<div class="row">
	<div class="col-lg-12">
			<h3 class="page-header">KOMPARASI PNS SIPD-SAPK</h3>
	</div>
</div>
								...
								<div class="col-lg-6">
									<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-list fa-fw"></span></button>
										<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
											...
											<li role="presentation" class="active"><a  role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-sitemap fa-fw"></i> Data NIP :: SAPK-Ada, SIPD-Tidak Ada</a></li>
										...
										<span>Data NIP :: SAPK-Ada, SIPD-Tidak Ada</span>
									</div>
								</div>
								<div class="col-lg-6">
										<a href="<?=site_url();?>csapk/cetak/sapk_ada" target="_blank"><div class="btn btn-primary btn-xs pull-right" id="bt_opsi" onclick="buka_div_opsi();"><i class="fa fa-print fa-fw"></i></div></a>
								</div>
						</div>
...
<th style="width:200px;text-align:center; vertical-align:middle">TMT CPNS / TMT PNS<br />STATUS KEPEGAWAIAN</th>
</tr>
</thead>
<tbody id="list">
</tbody>
...
var batas = $('#item_length').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>csapk/impor/getsapk",
	...
	//tombol aksi-->
						if(item.stt=="kosong"){
								table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>"+no+"</td>";
						} else {
								table = table+ "<td valign=top style='padding:3px 0px 0px 0px;' align=center>";
									...
									table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="detil('+item.id_jenjang_jabatan+',\'appbkpp/profile/pns_ini\',\'tidak\');return false;"><i class="fa fa-binoculars fa-fw"></i> Rincian Data Pegawai</a></li>';
									table = table+ '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" onclick="detil('+item.id_jenjang_jabatan+',\'appbina/absensi/pegawai\',\'ya\');return false;"><i class="fa fa-clock-o fa-fw"></i> Rincian Absensi Pegawai</a></li>';
									table = table+ "</ul>";
									table = table+ "</div>";
								table = table+ "</td>";
						}
	//tombol aksi<--
					...
					table = table+ '</div></td>';
					table = table+ "<td style='padding:3px;'>" +item.nomenklatur_jabatan+" (<b>"+item.tugas_tambahan+"</b>) <br/><u>pada</u><br/>"+item.nomenklatur_pada+"<br/><u>sejak</u>:<br> "+item.tmt_jabatan+"</td>";
					table = table+ "<td style='padding:3px;text-align:center'>"+item.tmt_cpns+"</br>"+item.tmt_pns+"</br><b>"+item.status_kepegawaian+"</b></td>";
					table = table+ "</tr>";
					no++;
