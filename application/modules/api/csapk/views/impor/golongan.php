										...
										<div class="col-lg-6">
											<div class="dropdown"><button class="btn btn-primary dropdown-toggle btn-xs" type="button" id="ddMenuT" data-toggle="dropdown"><span class="fa fa-list fa-fw"></span></button>
												<ul class="dropdown-menu" role="menu" aria-labelledby="ddMenuT">
													...
													<li class="divider"></li>
													<li role="presentation" class="active"><a role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-list fa-fw"></i> Data GOLONGAN :: SAPK-SIPD | Tidak Sama</a></li>
													<li role="presentation"><a href="<?=site_url('module/csapk/impor/jenis');?>" role="menuitem" tabindex="-1" style="cursor:pointer;"><i class="fa fa-list fa-fw"></i> Data JABATAN :: SAPK-SIPD | Tidak Sama</a></li>
												</ul>
												<span>Data GOLONGAN :: SAPK-SIPD | Tidak Sama</span>
											</div>
										</div>
										<div class="col-lg-6">
														<a href="<?=site_url();?>csapk/cetak/golongan" target="_blank"><div class="btn btn-primary btn-xs pull-right" id="bt_opsi" onclick="buka_div_opsi();"><i class="fa fa-print fa-fw"></i></div></a>
			
														<div class="btn-group pull-right" style="padding-right:5px;">
															<div class="btn btn-default btn-xs"  onclick="ganti_gol('kecil');" id="gol_kecil"><i class="fa fa-backward fa-fw"></i></div>
															<div class="btn btn-warning btn-xs" onclick="ganti_gol('all');" id="gol_all"> || </div>
															<div class="btn btn-default btn-xs"  onclick="ganti_gol('sama');" id="gol_sama"> = </div>
															<div class="btn btn-default btn-xs"  onclick="ganti_gol('besar');" id="gol_besar"><i class="fa fa-forward fa-fw"></i></div>
														</div>
														
														<div id="jenis_act" style="display:none;">all</div>
										</div>
								</div>
				</div><!-- /panel-heading -->
...
var batas = $('#item_length').val();
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>csapk/impor/getgolongan",
	...
	//tombol aksi<--
					...
					table = table+ item.nomenklatur_jabatan+"<br/><u>sejak</u>:<br> "+item.tmt_jabatan;
					table = table+ '</div>';
					table = table+ '</td>';
					table = table+ "<td style='padding:3px;'>";
					table = table+ '<div style="float:left; width:110px;">Golongan</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.nama_golongan+'</div>';
					table = table+ '</div>';
					table = table+ '<div style="clear:both;">';
					table = table+ '<div style="float:left; width:110px;">TMT Golongan</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.tmt_pangkat+'</div>';
					table = table+ '</div>';
					table = table+ '<div style="clear:both;">';
					table = table+ '<div style="float:left; width:110px;">MK.Golongan</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					...
					table = table+ '<div style="float:left; width:110px;">Golongan</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.nama_golongan_aktual+'</div>';
					table = table+ '</div>';
					table = table+ '<div style="clear:both;">';
					table = table+ '<div style="float:left; width:110px;">TMT Golongan</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.tmt_pangkat_aktual+'</div>';
					table = table+ '</div>';
					table = table+ '<div style="clear:both;">';
					table = table+ '<div style="float:left; width:110px;">MK.Golongan</div>';
					table = table+ '<div style="float:left; width:10px;">:</div>';
					table = table+ '<div style="float:left;">'+item.mk_gol_tahun_aktual+' tahun '+item.mk_gol_bulan_aktual+' bulan</div>';
					table = table+ '</td>';
					table = table+ "</tr>";
					no++;
...
function detil(idd,act,boleh){
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>"+act,
		data:{"idd": idd,"boleh":boleh,"awal":"sk_pangkat"},
		beforeSend:function(){	
...
function ganti_gol(pilihan){
	var jenis = $('#jenis_act').text();
	$('#gol_'+jenis).removeClass('btn-warning').addClass('btn-default');
	$('#gol_'+pilihan).removeClass('btn-default').addClass('btn-warning');
	$('#jenis_act').text(pilihan);
	gridpaging("end");
}
