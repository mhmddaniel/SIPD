...
//////////////////////////////////////////////////////////////////////////////////
    function tambah_aksi($isi){
		$sqlstr="INSERT INTO konten_appe (judul_appe,tipe,keterangan_appe,link) 
		VALUES ('".$isi['judul']."','sekilasinfo','".$isi['isi']."','".$isi['link']."')";		
		$this->db->query($sqlstr);
	}
    function edit_aksi($isi){
		$sqlstr="UPDATE konten_appe SET judul_appe='".$isi['judul']."',keterangan_appe='".$isi['isi']."',link='".$isi['link']."' WHERE id_appe='".$isi['idd']."'";
		$this->db->query($sqlstr);
	}
