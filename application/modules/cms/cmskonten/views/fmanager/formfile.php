...
?>
        <tr>
          <td><?=($key+1);?></td>
          <td>
<div class="dropdown"><button class="btn btn-default <?=($val->nilai==NULL || $val->nilai==0)?"dropdown-toggle":"";?> btn-xs" type="button" data-toggle="dropdown"><i class="fa fa-caret-down fa-fw"></i></button>
...
<div class="row" id="hapus_file" style="display:none;">
	<div class="col-lg-6">
		<div class="panel panel-success">
			...
			<div class="panel-body">
<div id="spinner_hapus"><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></div>
<div class="isiFhapus">File:</div>
<div class="isiFhapus" id='nmFile'></div>
<br><br>
...
<div class="row" id="rename_file" style="display:none;">
	<div class="col-lg-6">
		<div class="panel panel-success">
			...
			<div class="panel-body">
<div id="spinner_rename"><p class="text-center"><i class="fa fa-spinner fa-spin fa-5x"></i><p></div>
<div>Nama lama:</div>
<div id='nmFileR'></div>
<br>
<div class="renFR">Nama baru:</div>
<input type="text" name="baruF" id="baruF" class="form-control renFR" placeholder="Nama File Tanpa Ekstensi...">
<br><br>
...
function loadFL(tujuan,path,file,idd){
	...
	$.ajax({
		type:"POST",
		url:"<?=site_url();?>cmskonten/fmanager/cek_hapus/",
		data:{"idd":idd,"path":path,"file":file },
		beforeSend:function(){	
			$('#spinner_hapus').show();
			$('#btnHapus').hide();
		...
		success:function(data){
			...
			if(data=="Kosong"){
				...
				$('#btnHapus').attr('onclick','hapusFL(\''+tujuan+'\',\''+path+'\',\''+file+'\',\''+idd+'\');').show();
			} else {
				$('#nmFile').html(path+"<b style='color:#f00;'>"+file+"</b><br><br><b>Sudah dipakai, tidak dapat dihapus!!!</b>");
				$('#btnHapus').attr('onclick','hapusFL(\''+tujuan+'\',\''+path+'\',\''+file+'\',\''+idd+'\');').hide();
			}
		}, 
		dataType:"html"});
}
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data=="Kosong"){
				...
				$('#btnRename').attr('onclick','renameFL(\''+tujuan+'\',\''+path+'\',\''+file+'\',\''+idd+'\');').show();
			} else {
				$('#nmFileR').html(path+"<b style='color:#f00;'>"+file+"</b><br><br><b>Sudah dipakai, tidak dapat diganti nama!!!</b>");
				$('#btnRename').attr('onclick','renameFL(\''+tujuan+'\',\''+path+'\',\''+file+'\',\''+idd+'\');').hide();
			}
		}, 
		dataType:"html"});
}
