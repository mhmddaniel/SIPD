...
<nav class="navbar navbar-findcond navbar-fixed-top">
    ...
    <div class="navbar-header">
		...
		<div style="float:left;padding:5px 10px 0px 0px;"><img src="<?=base_url().$logo;?>" style='width:40px; vertical-align:middle;'></div>
    ...
    foreach ($nav as $key=>$val) {
		...
		if(isset($val->anak)){
			...
			foreach($val->anak AS $key2=>$val2){
				echo '<li role="presentation"><a role="menuitem" tabindex="-1" style="cursor:pointer;" href="'.site_url().$val2->path_menu.'"><i class="fa fa-'.$val2->icon_menu.' fa-fw"></i> '.$val2->nama_menu.'</a></li>';
			}
			echo '</ul></span>';
		} else {
			echo '<a href="'.site_url().$val->path_menu.'" class="btn btn-sq btn-'.$cActt.'"><i class="fa fa-'.$val->icon_menu.' fa-5x"></i><br><br/>'.$val->nama_menu.' </a>';
		} // end anak
    } // end foreach
} // end recKanal
?>
